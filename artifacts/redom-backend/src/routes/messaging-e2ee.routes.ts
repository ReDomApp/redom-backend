...
router.put("/crypto/device-key", async (req, res) => {
  ...
  if (!body.success) return void res.status(400).json({ success: false, message: "A valid device ID and X25519 public key are required." });
  ...
  const existing = await pool.query("SELECT primary_device FROM redom_device_crypto_keys WHERE device_id=$1 AND profile_id=$2 LIMIT 1", [body.data.deviceId, profileId]);
  const hasPrimary = await pool.query("SELECT 1 FROM redom_device_crypto_keys WHERE profile_id=$1 AND primary_device=true AND revoked_at IS NULL LIMIT 1", [profileId]);
  const primary = existing.rows[0]?.primary_device ?? hasPrimary.rows.length === 0;
  await pool.query(`INSERT INTO redom_device_crypto_keys(device_id, profile_id, public_key, algorithm, key_version, device_label, platform, primary_device, revoked_at, updated_at) VALUES ($1,$2,$3,'X25519-AES-256-GCM',1,$4,$5,$6,NULL,now()) ON CONFLICT(device_id) DO UPDATE SET public_key=EXCLUDED.public_key, algorithm=EXCLUDED.algorithm, key_version=EXCLUDED.key_version, device_label=EXCLUDED.device_label, platform=EXCLUDED.platform, revoked_at=NULL, updated_at=now()`, [body.data.deviceId, profileId, body.data.publicKey.toLowerCase(), body.data.deviceLabel ?? null, body.data.platform ?? null, primary]);
  res.json({ success: true, profileId, deviceId: body.data.deviceId, publicKey: body.data.publicKey.toLowerCase(), algorithm: "X25519-AES-256-GCM", keyVersion: 1, primaryDevice: primary });
});

router.get("/crypto/devices", async (req, res) => {
  if (!req.user?.userId) return void res.status(401).json({ success: false, message: "Authentication required." });
  const profileId = await profileIdFor(req.user.userId); if (!profileId) return void res.status(404).json({ success: false, message: "Profile not found." }); await ensureDeviceTable();
  const result = await pool.query("SELECT device_id, public_key, algorithm, key_version, device_label, platform, primary_device, created_at, updated_at FROM redom_device_crypto_keys WHERE profile_id=$1 AND revoked_at IS NULL ORDER BY primary_device DESC, updated_at DESC", [profileId]);
  res.json({ success: true, devices: result.rows });
});

router.delete("/crypto/devices/:deviceId", async (req, res) => {
  if (!req.user?.userId) return void res.status(401).json({ success: false, message: "Authentication required." });
  const deviceId = z.string().uuid().safeParse(req.params.deviceId); if (!deviceId.success) return void res.status(400).json({ success: false, message: "Invalid device." });
  const profileId = await profileIdFor(req.user.userId); if (!profileId) return void res.status(404).json({ success: false, message: "Profile not found." }); await ensureDeviceTable();
  const result = await pool.query("UPDATE redom_device_crypto_keys SET revoked_at=now(), updated_at=now() WHERE device_id=$1 AND profile_id=$2 AND revoked_at IS NULL RETURNING device_id", [deviceId.data, profileId]);
  if (!result.rows.length) return void res.status(404).json({ success: false, message: "Device not found." });
  res.json({ success: true, deviceId: deviceId.data, revoked: true });
});
...
router.get("/crypto/device-key/:profileId", async (req, res) => {
  ...
  const requester = await profileIdFor(req.user.userId); if (!requester) return void res.status(404).json({ success: false, message: "Profile not found." });
  if (requester !== profileId.data) { const exists = await db.select({ id: conversationParticipants.id }).from(conversationParticipants).where(and(eq(conversationParticipants.userId, requester), eq(conversationParticipants.activeMember, true), sql`EXISTS (SELECT 1 FROM conversation_participants cp2 WHERE cp2.conversation_id = ${conversationParticipants.conversationId} AND cp2.user_id = ${profileId.data} AND cp2.active_member = true)`)).limit(1); if (!exists.length) return void res.status(403).json({ success: false, message: "You cannot access this encryption identity." }); }
  await ensureDeviceTable(); const result = await pool.query("SELECT device_id, public_key, algorithm, key_version, updated_at FROM redom_device_crypto_keys WHERE profile_id=$1 AND revoked_at IS NULL ORDER BY primary_device DESC, updated_at DESC", [profileId.data]);
  if (!result.rows[0]) return void res.status(404).json({ success: false, message: "The recipient has not registered an encryption device." });
  res.json({ success: true, profileId: profileId.data, devices: result.rows });
});
...
router.patch("/conversations/:conversationId/messages/:messageId", async (req, res, next) => {
  ...
  const [updated] = await db.update(messages).set({ encryptedPayload: body.data.encryptedPayload, encryptionVersion: 1, encryptedAt: new Date(), edited: true, editedLabel: true, editedAt: new Date(), message: null, updatedAt: new Date() }).where(eq(messages.id, message.id)).returning(); res.json({ success: true, message: updated });
});
