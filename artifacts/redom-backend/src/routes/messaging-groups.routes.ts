...
async function member(conversationId: string, profile: string) {
  const [row] = await db.select().from(conversationParticipants).where(and(eq(conversationParticipants.conversationId, conversationId), eq(conversationParticipants.userId, profile), eq(conversationParticipants.activeMember, true), eq(conversationParticipants.temporarilySuspended, false), eq(conversationParticipants.permanentlyRemoved, false))).limit(1);
  return row ?? null;
}
...
router.patch("/groups/:conversationId/settings", async (req, res) => {
  ...
  const me = await profileId(req.user.userId); const actor = me ? await member(id.data, me) : null; if (!me || !actor) return void res.status(403).json({ success: false, message: "You do not have access to this group." });
  ...
  if ((infoFields || permissionFields) && !actor) return void res.status(403).json({ success: false, message: "Group membership required." });
  if (infoFields && !body.data.groupName && body.data.groupDescription === undefined) return void res.status(400).json({ success: false, message: "Group name cannot be empty." });
  if (infoFields && !actor) return void res.status(403).json({ success: false, message: "Group membership required." });
  const [group] = await db.select({ anyoneCanEditInfo: conversations.anyoneCanEditInfo }).from(conversations).where(eq(conversations.id, id.data)).limit(1);
  ...
  await db.update(conversations).set({ ...body.data, updatedAt: new Date() }).where(eq(conversations.id, id.data));
...
router.post("/groups/:conversationId/members", async (req, res) => {
  ...
  const me = await profileId(req.user.userId); const actor = me ? await member(id.data, me) : null; if (!me || !actor) return void res.status(403).json({ success: false, message: "You do not have access to this group." });
  const [group] = await db.select({ anyoneCanInvite: conversations.anyoneCanInvite, participantCount: conversations.participantCount }).from(conversations).where(eq(conversations.id, id.data)).limit(1); if (!group) return void res.status(404).json({ success: false, message: "Group not found." });
  ...
  const existing = await db.select({ userId: conversationParticipants.userId }).from(conversationParticipants).where(and(eq(conversationParticipants.conversationId, id.data), inArray(conversationParticipants.userId, requested)));
  const existingSet = new Set(existing.map(r => r.userId)); const add = requested.filter(p => !existingSet.has(p)); if (!add.length) return res.json({ success: true, added: 0 });
  await db.insert(conversationParticipants).values(add.map(userId => ({ conversationId: id.data, userId, role: "member", joinedBy: me, joinRequestApproved: !group.participantCount }))); await db.update(conversations).set({ participantCount: group.participantCount + add.length, updatedAt: new Date() }).where(eq(conversations.id, id.data));
  res.status(201).json({ success: true, added: add.length });
});
...
router.delete("/groups/:conversationId/members/:profileId", async (req, res) => {
  ...
  const me = await profileId(req.user.userId); const actor = me ? await member(id.data, me) : null; const targetMember = await member(id.data, target.data); if (!me || !actor || !targetMember) return void res.status(403).json({ success: false, message: "Member access denied." });
  ...
  if (!group.anyoneCanRemoveMembers && !canAdmin(actor.role)) return void res.status(403).json({ success: false, message: "Only group admins can remove members." });
  if (targetMember.role === "owner") return void res.status(403).json({ success: false, message: "The group owner cannot be removed." });
  await db.update(conversationParticipants).set({ activeMember: false, permanentlyRemoved: true, removedByAdmin: canAdmin(actor.role), leftAt: new Date(), updatedAt: new Date() }).where(eq(conversationParticipants.id, targetMember.id));
  await db.update(conversations).set({ participantCount: Math.max(0, group.participantCount - 1), updatedAt: new Date() }).where(eq(conversations.id, id.data));
...
router.patch("/groups/:conversationId/members/:profileId/role", async (req, res) => {
  ...
  const me = await profileId(req.user.userId); const actor = me ? await member(id.data, me) : null; const targetMember = await member(id.data, target.data); if (!me || !actor || !targetMember || actor.role !== "owner") return void res.status(403).json({ success: false, message: "Only the group owner can manage admin roles." });
...
router.post("/groups/:conversationId/leave", async (req, res) => {
  const id = z.string().uuid().safeParse(req.params.conversationId); if (!req.user?.userId || !id.success) return void res.status(400).json({ success: false, message: "Invalid group." });
  const me = await profileId(req.user.userId); const actor = me ? await member(id.data, me) : null; if (!me || !actor) return void res.status(403).json({ success: false, message: "You are not a group member." });
  if (actor.role === "owner") return void res.status(409).json({ success: false, message: "Transfer group ownership before leaving." });
  await db.update(conversationParticipants).set({ activeMember: false, leftGroup: true, leftAt: new Date(), updatedAt: new Date() }).where(eq(conversationParticipants.id, actor.id)); await db.update(conversations).set({ participantCount: Math.max(0, (await db.select({ count: conversations.participantCount }).from(conversations).where(eq(conversations.id, id.data)).limit(1))[0]?.count ?? 1 - 1), updatedAt: new Date() }).where(eq(conversations.id, id.data)); res.json({ success: true, left: true });
});
