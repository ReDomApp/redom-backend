...
const POLICY_DOCUMENTS: Record<string, { title: string; summary: string; sections: Array<{ heading: string; body: string }> }> = {
  terms: { title: "ReDom Terms of Use", summary: "The rules governing access to and use of ReDom services.", sections: [
  ...
  privacy: { title: "ReDom Privacy Policy", summary: "How ReDom collects, uses, protects and shares information.", sections: [
  ...
  community: { title: "ReDom Community Standards", summary: "Safety and conduct standards for the ReDom community.", sections: [
  ...
  messaging: { title: "ReDom Messaging Policy", summary: "Privacy, safety and controls for ReDom conversations and messages.", sections: [
  ...
  notifications: { title: "ReDom Notification Policy", summary: "How ReDom notification events and controls work.", sections: [
  ...
  security: { title: "ReDom Security Policy", summary: "Account, device, authentication and platform security controls.", sections: [
  ...
  verification: { title: "ReDom Verification Policy", summary: "Eligibility, security and review principles for account verification.", sections: [
  ...
  ai: { title: "ReDom AI Policy", summary: "How ReDom AI can assist without bypassing platform controls.", sections: [
  ...
  regional: { title: "ReDom Regional Policy", summary: "Country and region-specific availability and policy controls.", sections: [
  ...
  refunds: { title: "ReDom Refund Policy", summary: "Product and country-specific rules for supported refund requests.", sections: [
  ...
  support: { title: "ReDom Support and Reporting Policy", summary: "How users can get help, report problems and follow supported cases.", sections: [
...
router.get("/policies/:slug", (req, res) => {
  ...
  if (!document) { res.status(404).json({ success: false, code: "POLICY_NOT_FOUND", message: "Policy not found." }); return; }
...
router.get("/notifications", authMiddleware, async (req, res) => {
  ...
  const rows = await db.select().from(notifications).where(and(eq(notifications.recipientUserId, profileId), eq(notifications.deleted, false))).orderBy(desc(notifications.createdAt)).limit(100);
...
router.post("/notifications/:id/read", authMiddleware, async (req, res) => {
  ...
  if (!profileId) { res.status(404).json({ success: false, message: "Profile not found." }); return; }
  if (!id.success) { res.status(400).json({ success: false, message: "Invalid notification id." }); return; }
  await db.update(notifications).set({ unread: false, read: true, readAt: new Date(), updatedAt: new Date() }).where(and(eq(notifications.id, id.data), eq(notifications.recipientUserId, profileId)));
  await db.insert(activityLog).values({ userId: req.user.userId, activityType: "notification_opened", activityCategory: "notifications", activityTitle: "Notification opened", activityDescription: "A ReDom notification was opened.", targetId: id.data, targetType: "notification", status: "success", triggeredBy: "user", source: "app", undoSupported: false, hidden: false, archived: false });
...
router.post("/notifications/read-all", authMiddleware, async (req, res) => {
  ...
  const profileId = await currentProfileUuid(req.user.userId);
  if (!profileId) { res.status(404).json({ success: false, message: "Profile not found." }); return; }
  await db.update(notifications).set({ unread: false, read: true, readAt: new Date(), updatedAt: new Date() }).where(and(eq(notifications.recipientUserId, profileId), eq(notifications.deleted, false), eq(notifications.unread, true)));
  await db.insert(activityLog).values({ userId: req.user.userId, activityType: "notifications_marked_read", activityCategory: "notifications", activityTitle: "Notifications marked read", activityDescription: "Unread ReDom notifications were marked as read.", status: "success", triggeredBy: "user", source: "app", undoSupported: false, hidden: false, archived: false });
  res.json({ success: true });
});
...
router.patch("/settings", authMiddleware, async (req, res) => {
  ...
  const parsed = z.object({
    theme: z.enum(["system", "light", "dark"]).optional(), language: z.string().min(2).max(20).optional(), politicalContent: z.boolean().optional(), followingFeed: z.boolean().optional(), followingFeedSnooze: z.enum(["off", "1h", "8h", "24h", "7d"]).optional(), sensitiveContent: z.enum(["standard", "less", "more"]).optional(), autoplayVideos: z.enum(["off", "wifi", "always"]).optional(), autoTranslatePosts: z.boolean().optional(), autoTranslateComments: z.boolean().optional(), fontSize: z.enum(["small", "medium", "large"]).optional(), reduceMotion: z.boolean().optional(), highContrast: z.boolean().optional(), screenReaderMode: z.boolean().optional(), captions: z.enum(["automatic", "on", "off"]).optional(), showJoinDate: z.boolean().optional(),
  }).strict().safeParse(req.body);
  if (!parsed.success || Object.keys(parsed.data).length === 0) { res.status(400).json({ success: false, message: "No valid settings were supplied." }); return; }
  const [existing] = await db.select().from(userSettings).where(eq(userSettings.userId, req.user.userId)).limit(1);
  const now = new Date();
  if (existing) await db.update(userSettings).set({ ...parsed.data, updatedAt: now }).where(eq(userSettings.userId, req.user.userId));
  else await db.insert(userSettings).values({ userId: req.user.userId, ...parsed.data, updatedAt: now, createdAt: now });
