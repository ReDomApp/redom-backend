...
const forwardItemSchema = z.object({ conversationId: z.string().uuid(), encryptedPayload: encryptedPayloadSchema }).strict();
const bodySchema = z.object({ sourceMessageId: z.string().uuid(), forwards: z.array(forwardItemSchema).min(1).max(5) }).strict();
...
router.post("/forward", async (req, res) => {
  ...
  const viewOnce = await pool.query("SELECT 1 FROM message_lifecycle WHERE message_id=$1 AND view_once=true LIMIT 1", [source.id]).catch(() => ({ rows: [] as unknown[] }));
  if (viewOnce.rows.length) return void res.status(403).json({ success: false, message: "View Once messages cannot be forwarded." });

  const depthResult = await pool.query(`WITH RECURSIVE chain AS (SELECT id, forwarded_from_message_id, 0 AS depth FROM messages WHERE id=$1 UNION ALL SELECT m.id, m.forwarded_from_message_id, c.depth + 1 FROM messages m JOIN chain c ON m.id=c.forwarded_from_message_id WHERE c.depth < 10) SELECT COALESCE(MAX(depth),0)::int AS depth FROM chain`, [source.id]);
  ...
  const destinationRows = await db.select({ id: conversations.id, conversationType: conversations.conversationType, status: conversations.status, locked: conversations.locked, deleted: conversations.deleted }).from(conversations).where(inArray(conversations.id, destinationIds));
  ...
  for (const destination of body.data.forwards) {
    ...
    for (const deviceId of Object.keys(destination.encryptedPayload)) if (!allowed.has(deviceId)) return void res.status(400).json({ success: false, message: "An encrypted recipient is not an active device in one of the selected chats." });
    ...
    const [created] = await db.insert(messages).values({ conversationId: destination.conversationId, senderId: profileId, parentMessageId: null, forwardedFromMessageId: source.id, isForwarded: true, messageType: source.messageType, message: null, caption: null, sent: true, delivered: false, read: false, aiReviewed: false, moderationStatus: "approved", encryptedPayload: destination.encryptedPayload, encryptionVersion: 1, encryptedAt: new Date() }).returning();
    ...
    const recipients = await db.select({ userId: conversationParticipants.userId, notificationsEnabled: conversationParticipants.notificationsEnabled, muted: conversationParticipants.muted }).from(conversationParticipants).where(and(eq(conversationParticipants.conversationId, destination.conversationId), eq(conversationParticipants.activeMember, true), eq(conversationParticipants.temporarilySuspended, false), eq(conversationParticipants.permanentlyRemoved, false), sql`${conversationParticipants.userId} <> ${profileId}`));
