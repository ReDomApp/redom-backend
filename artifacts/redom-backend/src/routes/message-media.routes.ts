...
function parseDataUri(value: string) {
  ...
  if (!body.length || body.length > MAX_BYTES) throw new Error("Message media must be 10 MB or smaller.");
  return { mime, body };
}
...
router.get(/^\/media\/(.+)$/, authMiddleware, async (req, res) => {
  ...
  if (!key.startsWith("messages/") || key.includes("..")) return void res.status(400).end();
  const conversationId = key.split("/")[1];
  const parsedConversationId = z.string().uuid().safeParse(conversationId);
  if (!parsedConversationId.success || !req.user?.userId) return void res.status(403).end();
  const profileId = await profileIdFor(req.user.userId);
  if (!profileId) return void res.status(404).end();
  const [member] = await db.select({ id: conversationParticipants.id }).from(conversationParticipants).where(and(eq(conversationParticipants.conversationId, parsedConversationId.data), eq(conversationParticipants.userId, profileId), eq(conversationParticipants.activeMember, true), eq(conversationParticipants.temporarilySuspended, false), eq(conversationParticipants.permanentlyRemoved, false))).limit(1);
  if (!member) return void res.status(403).end();
  const [attachment] = await db.select({ id: messageAttachments.id, messageId: messageAttachments.messageId }).from(messageAttachments).where(and(eq(messageAttachments.fileUrl, `/messages/media/${key}`), eq(messageAttachments.active, true))).limit(1);
  if (!attachment) return void res.status(404).end();
  const [message] = await db.select({ id: messages.id, deletedForEveryone: messages.deletedForEveryone }).from(messages).where(and(eq(messages.id, attachment.messageId), eq(messages.conversationId, parsedConversationId.data))).limit(1);
  if (!message || message.deletedForEveryone) return void res.status(404).end();
  try {
    const object = await r2.send(new GetObjectCommand({ Bucket: env.cloudflare.r2.bucketName, Key: key }));
    if (!object.Body) return void res.status(404).end();
    res.setHeader("Cache-Control", "private, max-age=300");
    res.setHeader("Content-Type", object.ContentType || "application/octet-stream");
    return res.end(Buffer.from(await object.Body.transformToByteArray()));
  } catch { return void res.status(404).end(); }
});
...
router.post("/conversations/:conversationId/media", async (req, res) => {
  ...
  const [conversation] = await db.select().from(conversations).where(and(eq(conversations.id, conversationId.data), eq(conversations.deleted, false), eq(conversations.locked, false), eq(conversations.status, "active"))).limit(1);
  ...
  if (parsed.data.parentMessageId) {
    const [parent] = await db.select({ id: messages.id }).from(messages).where(and(eq(messages.id, parsed.data.parentMessageId), eq(messages.conversationId, conversationId.data), eq(messages.deletedForEveryone, false))).limit(1);
    if (!parent) return void res.status(400).json({ success: false, message: "The reply target is unavailable." });
  }
  try {
    const media = parseDataUri(parsed.data.dataUri);
    const extension = mimeExt[media.mime] || (media.mime.split("/")[1] || "bin").replace(/[^a-z0-9]+/gi, "").slice(0, 10) || "bin";
    const key = `messages/${conversationId.data}/${randomUUID()}.${extension}`;
    ...
    for (const recipient of recipients) {
      await db.update(conversationParticipants).set({ unreadMessageCount: sql`${conversationParticipants.unreadMessageCount} + 1` }).where(and(eq(conversationParticipants.conversationId, conversationId.data), eq(conversationParticipants.userId, recipient.userId)));
