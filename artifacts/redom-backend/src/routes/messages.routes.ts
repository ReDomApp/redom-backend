...
router.get("/conversations", authMiddleware, authRateLimit, async (req, res) => {
  ...
  const latest = await Promise.all(rows.map(async (c) => {
    ...
    return { ...c, unreadMessageCount: membership?.unreadMessageCount ?? 0, muted: membership?.muted ?? false, pinned: membership?.pinned ?? false, archived: membership?.archived ?? false, notificationsEnabled: membership?.notificationsEnabled ?? true, mentionsOnly: membership?.mentionsOnly ?? false, lastMessage: last ? { ...last, message: null } : null };
  }));
...
router.post("/conversations/direct", authMiddleware, authRateLimit, async (req, res) => {
  ...
  const mine = await db.select({ conversationId: conversationParticipants.conversationId }).from(conversationParticipants).innerJoin(conversations, eq(conversations.id, conversationParticipants.conversationId)).where(and(eq(conversationParticipants.userId, profileId), eq(conversationParticipants.activeMember, true), eq(conversations.conversationType, "direct"), eq(conversations.deleted, false), eq(conversations.status, "active")));
  for (const row of mine) {
    const [other] = await db.select({ id: conversationParticipants.id }).from(conversationParticipants).where(and(eq(conversationParticipants.conversationId, row.conversationId), eq(conversationParticipants.userId, parsed.data.recipientProfileId), eq(conversationParticipants.activeMember, true), eq(conversationParticipants.temporarilySuspended, false), eq(conversationParticipants.permanentlyRemoved, false))).limit(1);
    if (other) return void res.json({ success: true, conversationId: row.conversationId, existing: true });
  }
  const [conversation] = await db.insert(conversations).values({ createdBy: profileId, conversationType: "direct", encrypted: true, aiModerationEnabled: false }).returning({ id: conversations.id });
...
// Text send/edit are owned by messaging-e2ee.routes.ts, mounted before this router.
router.post("/conversations/:conversationId/messages", authMiddleware, authRateLimit, async (_req, _res, next) => next());
router.patch("/conversations/:conversationId/messages/:messageId", authMiddleware, authRateLimit, async (_req, _res, next) => next());
