...
export const registrationFlowReservations = pgTable(
  ...
  (table) => ({
    ...
    phoneLookupCountryFormat: check(
      "registration_flow_reservations_phone_lookup_country_chk",
      sql`${table.phoneLookupCountryCode} is null or ${table.phoneLookupCountryCode} ~ '^[A-Z]{2}$'`,
    ),
    ipCountryFormat: check(
      "registration_flow_reservations_ip_country_chk",
      sql`${table.ipCountryCode} is null or ${table.ipCountryCode} ~ '^[A-Z]{2}$'`,
    ),
