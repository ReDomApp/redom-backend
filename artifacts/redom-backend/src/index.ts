...
import { ensureMessagingCompletionSchema } from "./services/messaging-completion-bootstrap";
...
void start().catch((error) => {
  logger.error({ error: serializeError(error) }, "Unable to start ReDom backend");
  process.exit(1);
});
