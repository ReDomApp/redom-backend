...
function categoryNames(categories: Record<string, boolean>) { return Object.entries(categories).filter(([, value]) => value).map(([key]) => key); }
...
async function notifyReporter(reportId: string, result: { status: string; decision: string; categories: string[]; evidenceCount: number; groupName: string; reason: string; exitAfterReport: boolean }) {
  ...
  const sentAt = new Date(); const categoryText = result.categories.length ? result.categories.join(", ") : "No harmful-content category was detected"; const subject = `ReDom report ${reportId.slice(0, 8)} — ${result.status.replace(/_/g, " ")}`;
  ...
  try { await resend.emails.send({ from: env.email.reportsFrom, to: [recipient.email], subject, html }); await db.update(reports).set({ emailNotificationSentAt: sentAt, emailNotificationError: null, updatedAt: sentAt }).where(eq(reports.id, reportId)); }
  catch (error) { const message = error instanceof Error ? error.message.slice(0, 2000) : "Unable to send report notification."; await db.update(reports).set({ emailNotificationError: message, updatedAt: new Date() }).where(eq(reports.id, reportId)); }
}
...
export async function moderateGroupReport(reportId: string) {
  const [report] = await db.select({ id: reports.id, reporterUserId: reports.reporterUserId, reportReason: reports.reportReason, exitAfterReport: reports.exitAfterReport, conversationId: reports.targetId }).from(reports).where(eq(reports.id, reportId)).limit(1);
