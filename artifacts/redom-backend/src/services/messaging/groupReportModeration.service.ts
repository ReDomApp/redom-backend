...
export async function moderateGroupReport(reportId: string) {
  ...
  try {
    if (moderationTexts.length) {
      ...
      scores = rawScores;
      if (result?.flagged) { status = "closed"; decision = "violation_detected"; autoRemoved = true; autoHidden = true; requiresHumanReview = false; }
      else { status = "closed"; decision = "no_violation_detected"; requiresHumanReview = false; }
    }
  } catch (error) {
    decision = "ai_review_failed"; status = "under_review"; requiresHumanReview = true;
