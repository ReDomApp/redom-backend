...
async function activeReservation(reservationId: string | undefined, flowId: string, deviceId?: string) {
  const reservation = await db.query.registrationFlowReservations.findFirst({
    where: and(
      reservationId ? eq(registrationFlowReservations.id, reservationId) : eq(registrationFlowReservations.flowId, flowId),
...
export class RegistrationVerificationService {
  ...
  async switchContact(params: { verificationChallengeId: string; reservationId?: string; flowId: string; deviceId?: string; contactType: ContactType; target: string }) {
    ...
    if (params.contactType === "email") {
      ...
      await db.update(registrationFlowReservations).set({ memory, registeredTables: [...new Set([...(reservation.registeredTables ?? []), "registration_flow_reservations", "email"])] as string[] }).where(and(eq(registrationFlowReservations.id, reservation.id), eq(registrationFlowReservations.flowId, reservation.flowId), gt(registrationFlowReservations.expiresAt, new Date())));
      channel = "email";
    } else {
    ...
    const created = await verificationService.createVerification({ userId: user.id, purpose, target: normalizedTarget, channel, requestedLength: 6, firstName: user.firstName ?? undefined, deviceId: params.deviceId, sessionId: reservation.id });
