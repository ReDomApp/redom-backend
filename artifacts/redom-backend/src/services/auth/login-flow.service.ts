...
export interface LoginFlowResult {
  ...
  twoFactorVerification?: LoginVerification;
}

type PublicUser = {
  id: string; username: string; publicId: string; profileId: string;
  firstName: string; lastName: string; email: string | null; phoneNumber: string | null;
  emailVerified: boolean; phoneVerified: boolean; accountStatus: string;
};
type LoginSession = { sessionId: string; accessToken: string; refreshToken: string; expiresAt: Date };
type LoginVerification = { challengeId: string; channel: "sms" | "email" | "whatsapp"; target: string; maskedTarget: string; codeLength: number; expiresAt: Date };
...
export class LoginFlowService {
  private async inspectIp(ipAddress?: string): Promise<IPAPIResult | null> {
    if (!ipAddress) return null;
    try { return await checkIP(ipAddress); } catch { return null; }
  }

  private async securitySettings(userId: string) {
    return db.query.accountSecurity.findFirst({ where: eq(accountSecurity.userId, userId) });
  }
  ...
  private async createTwoFactorChallenge(user: typeof users.$inferSelect, security: typeof accountSecurity.$inferSelect, request: { ipAddress?: string; userAgent?: string; deviceId?: string }) : Promise<LoginVerification> {
    ...
    let target: string;
    if (security.twoFactorMethod === "email") {
      if (!user.email || !user.emailVerified) throw new Error("Two-factor authentication requires a verified email address.");
      channel = "email"; target = user.email;
    } else if (security.twoFactorMethod === "phone") {
      if (!user.phoneNumber || !user.phoneVerified) throw new Error("Two-factor authentication requires a verified phone number.");
      channel = "sms"; target = user.phoneNumber;
    } else {
      throw new Error("Authenticator-app two-factor authentication is not yet available in the login flow.");
    }
  ...
  private async finishLogin(user: typeof users.$inferSelect, data: LoginRequest, ipapi: IPAPIResult | null, deviceId?: string) {
    ...
    if (user.email) {
      try { await loginNotificationService.send({ email: user.email, firstName: user.firstName, lastName: user.lastName, ipAddress: data.ipAddress ?? "Unknown", eventAt: new Date(), deviceUserAgent: data.userAgent, ipapi }); } catch { /* Login remains successful if notification delivery fails. */ }
    }
    return session;
  }
  ...
  private async completeAfterSecurity(user: typeof users.$inferSelect, data: LoginRequest, deviceId?: string, ipapi?: IPAPIResult | null): Promise<LoginFlowResult> {
    const security = await this.securitySettings(user.id);
    const network = ipapi ?? await this.inspectIp(data.ipAddress);
    if (security?.twoFactorEnabled) {
      const twoFactorVerification = await this.createTwoFactorChallenge(user, security, { ipAddress: data.ipAddress, userAgent: data.userAgent, deviceId });
      return { success: true, message: "Two-factor authentication is required.", requiresVerification: false, requiresTwoFactor: true, user: publicUser(user), twoFactorVerification };
    }
    const session = await this.finishLogin(user, data, network, deviceId);
    return { success: true, message: "Login successful.", requiresVerification: false, requiresTwoFactor: false, user: publicUser(user), session };
  }

  async login(data: LoginRequest): Promise<LoginFlowResult> {
    const identifier = data.identifier.trim();
    if (!identifier) throw new Error("Login identifier is required.");
  ...
  async verifyNewDevice(params: { challengeId: string; code: string; ipAddress?: string; deviceId: string; deviceName?: string; deviceType?: string; platform?: string; browser?: string; userAgent?: string; country?: string; region?: string; city?: string; loginSource?: string; appVersion?: string; }) {
    ...
    const verification = await verificationService.verifyVerification({ challengeId: params.challengeId, code: params.code, purpose: "LOGIN_DEVICE_VERIFICATION" });
    if (!verification.userId) throw new Error("Verification is not associated with an account.");
    ...
    const ipapi = await this.inspectIp(params.ipAddress);
    await fraudService.checkLogin({ userId: user.id, ipAddress: params.ipAddress, country: ipapi?.location?.country ?? params.country, userAgent: params.userAgent });
  ...
  async verifyTwoFactor(params: { challengeId: string; code: string; deviceId: string; deviceName?: string; deviceType?: string; platform?: string; browser?: string; userAgent?: string; ipAddress?: string; country?: string; region?: string; city?: string; loginSource?: string; appVersion?: string; }) {
    const challenge = await db.query.verifications.findFirst({ where: eq(verifications.id, params.challengeId) });
    if (!challenge) throw new Error("Two-factor verification challenge not found.");
    ...
    const verification = await verificationService.verifyVerification({ challengeId: params.challengeId, code: params.code, purpose: "TWO_FACTOR_AUTHENTICATION" });
    if (!verification.userId) throw new Error("Two-factor verification is not associated with an account.");
    const user = await db.query.users.findFirst({ where: eq(users.id, verification.userId) });
    if (!user) throw new Error("User not found.");
    const data: LoginRequest = { identifier: user.email ?? user.phoneNumber ?? "", password: "", ipAddress: params.ipAddress, country: params.country, region: params.region, city: params.city, userAgent: params.userAgent, platform: params.platform, browser: params.browser, deviceName: params.deviceName, deviceId: params.deviceId, deviceType: params.deviceType, loginSource: params.loginSource ?? "mobile", appVersion: params.appVersion };
    const ipapi = await this.inspectIp(params.ipAddress);
