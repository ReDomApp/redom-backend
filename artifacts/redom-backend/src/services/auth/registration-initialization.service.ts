...
export class RegistrationInitializationService {
  async initialize(params: {
    ...
    if (!verification || !["EMAIL_VERIFICATION", "PHONE_VERIFICATION"].includes(verification.purpose)) {
    ...
    const reservation = await db.query.registrationFlowReservations.findFirst({
      where: and(eq(registrationFlowReservations.flowId, challenge.flowId), eq(registrationFlowReservations.status, "active")),
    });
    ...
    if (!user) {
      ...
      user = await db.query.users.findFirst({
        where: email ? eq(users.email, email) : phone ? eq(users.phoneNumber, phone) : eq(users.username, challenge.username!),
      });
    }
    ...
    if (ip) {
      try { geo = await checkIP(ip); } catch { geo = null; }
    }
    ...
    const [profile] = await db.insert(userProfiles).values({
      ...
      postCount: 0,
      updatedAt: new Date(),
    }).onConflictDoNothing({ target: userProfiles.userId }).returning();
    ...
    if (!existingSettings) {
      await db.insert(userSettings).values({ userId: user.id, language: params.language?.trim() || "system", updatedAt: new Date() });
    } else if (params.language?.trim()) {
      await db.update(userSettings).set({ language: params.language.trim(), updatedAt: new Date() }).where(eq(userSettings.userId, user.id));
    }
    ...
    const session = await sessionService.createSession({
      ...
      city: city ?? undefined,
      userAgent: params.userAgent ?? verification.userAgent ?? challenge.userAgent ?? undefined,
      platform: params.platform,
      browser: params.browser,
      deviceName: params.deviceName,
      deviceId: params.deviceId ?? verification.deviceId ?? challenge.deviceId ?? undefined,
      deviceType: params.deviceType,
      loginSource: params.loginSource ?? "registration",
    ...
    const now = new Date();
    const setupMemory = { setup: { status: "completed", userId: user.id, sessionId: session.sessionId, profileId: user.profileId, publicId: user.publicId, language: params.language?.trim() || "system", completedAt: now.toISOString() } };
