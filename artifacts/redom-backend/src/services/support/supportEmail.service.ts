...
export async function sendRefundCaseEmail(input: {
  ...
  if (input.terminal) {
    ...
    const { error } = await resend.emails.send({from: input.from ?? env.email.supportFrom,to:[input.to],subject:"ReDom Refund — "+input.status+" — "+input.transactionNumber,text,html});
