...
export const env = {
  ...
  cloudflare: {
    r2: {
      ...
      apiToken: required("R2_API_TOKEN"),
      accessKeyId: required("R2_ACCESS_KEY_ID"),
      secretAccessKey: required("R2_SECRET_ACCESS_KEY"),
      bucketName: required("R2_BUCKET_NAME"),
      endpoint: required("R2_ENDPOINT"),
      bucketEndpoint: required("R2_BUCKET_ENDPOINT"),
      region: required("R2_REGION"),
    },
    stream: {
      accountId: required("STREAM_ACCOUNT_ID"),
      customerSubdomain: required("STREAM_CUSTOMER_SUBDOMAIN"),
    },
    turnstile: {
  ...
  oneSignal: {
    appId: required("ONESIGNAL_APP_ID"),
    restApiKey: required("ONESIGNAL_REST_API_KEY"),
  },
  redis: {
    url: required("REDIS_URL"),
    token: required("REDIS_TOKEN"),
    endpoint: required("REDIS_ENDPOINT"),
    port: requiredNumber("REDIS_PORT"),
  },
  qstash: {
    url: required("QSTASH_URL"),
  ...
  meilisearch: {
    ...
    masterKey: required("MEILISEARCH_MASTER_KEY"),
    adminApiKey: required("MEILISEARCH_ADMIN_API_KEY"),
    searchApiKey: required("MEILISEARCH_SEARCH_API_KEY"),
    readonlyApiKey: required("MEILISEARCH_READONLY_API_KEY"),
    chatApiKey: required("MEILISEARCH_CHAT_API_KEY"),
  },
  ipApi: { apiKey: required("IPAPI_API_KEY") },
  abstract: { apiKey: optional("ABSTRACT_API_KEY") },
  maxMind: {
    accountId: required("MAXMIND_ACCOUNT_ID"),
    licenseKey: required("MAXMIND_LICENSE_KEY"),
  },
  mapbox: {
    username: required("MAPBOX_USERNAME"),
    accessToken: required("MAPBOX_ACCESS_TOKEN"),
    secretToken: required("MAPBOX_SECRET_TOKEN"),
  },
