...
export const env = {
  ...
  email: {
    ...
    bugsFrom: optional("BUGS_EMAIL_FROM") ?? "bugs@wnncompany.com",
    problemFrom: optional("PROBLEM_EMAIL_FROM") ?? "problem@wnncompany.com",
    bugReportRecipients: (optional("BUG_REPORT_RECIPIENTS") ?? "admin@wnncompany.com,christianuzamaosahuo@gmail.com").split(",").map((value) => value.trim()).filter(Boolean),
