...
// Public profile URLs are intentionally outside authentication. They must be
// readable by WhatsApp/Facebook/etc. crawlers so those services can build a
// preview, while Android/iOS App Links can route the same HTTPS URL into ReDom.
app.use(profileShareRouter);

// Keep the public /redom-backend prefix used by the mobile client. The root
// mount also supports managed proxies that strip that prefix before Express.
