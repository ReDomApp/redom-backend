...
export function ChatEncryptionNotice({ visible = true }: Props) {
  ...
  return <>
    <Pressable accessibilityRole="button" accessibilityLabel="End-to-end encryption information" onPress={() => setOpen(true)} style={styles.banner}>
      <ChatInfoIcon name="encryption" size={15} color="#344054" />
      <Text style={styles.bannerText}>Messages and calls are end-to-end encrypted. Only people in this chat can read, listen to, or share them. <Text style={styles.bannerLink}>Learn more</Text></Text>
    </Pressable>
    <Modal visible={open} animationType="slide" transparent onRequestClose={() => setOpen(false)}>
      <SafeAreaView style={styles.overlay}>
        <View style={styles.sheet}>
          <View style={styles.handle} />
          <Pressable accessibilityRole="button" accessibilityLabel="Close encryption information" onPress={() => setOpen(false)} style={styles.close}><NavigationIcon kind="close" size={30} color="#667085" /></Pressable>
          <View style={styles.illustration}>
            <View style={styles.safeBody}><View style={styles.safeDial}><ChatInfoIcon name="encryption" size={38} color="#101828" /></View><View style={styles.safeLatch} /></View>
          ...
          <Text style={styles.description}>End-to-end encryption keeps your personal messages and calls between you and the people you choose. No one outside of the chat, not even ReDom, can read, listen to, or share them. This includes your:</Text>
          <View style={styles.details}>{DETAILS.map((item) => <View key={item.label} style={styles.detailRow}><ChatInfoIcon name={item.icon} size={25} color="#667085" /><Text style={styles.detailText}>{item.label}</Text></View>)}</View>
          <Pressable accessibilityRole="button" accessibilityLabel="Open ReDom messaging policy" onPress={() => { setOpen(false); navigation.navigate("Policy", { slug: "messaging" }); }} style={styles.learnMore}><Text style={styles.learnMoreText}>Learn more</Text></Pressable>
        </View>
      </SafeAreaView>
    </Modal>
  </>;
...
const styles = StyleSheet.create({
  banner: { marginHorizontal: 24, marginTop: 10, marginBottom: 8, paddingHorizontal: 14, paddingVertical: 12, borderRadius: 10, backgroundColor: "#EEF4FF", flexDirection: "row", alignItems: "flex-start" },
  bannerText: { flex: 1, marginLeft: 7, color: "#344054", fontSize: 14, lineHeight: 19, textAlign: "center" },
  bannerLink: { fontWeight: "800", color: "#1877F2" },
  ...
  safeBody: { width: 150, height: 122, borderRadius: 25, backgroundColor: "#1877F2", borderWidth: 2, borderColor: "#101828", alignItems: "center", justifyContent: "center", transform: [{ rotate: "-1deg" }] },
