...
export function CallScreen({ route, navigation }: Props) {
  ...
  const peer = useRef<RTCPeerConnection | null>(null); const stream = useRef<any>(null); const timer = useRef<ReturnType<typeof setInterval> | null>(null); const last = useRef<string | undefined>(undefined);
  ...
  useEffect(() => { let alive = true;
    const start = async () => { try {
      ...
      const pc = new RTCPeerConnection({ iceServers: ICE_SERVERS }) as any; peer.current = pc;
      pc.addEventListener("icecandidate", (e: any) => { if (e.candidate) void messageService.sendCallSignal(active.id, "ice", e.candidate.toJSON()); }); pc.addEventListener("track", (e: any) => { if (e.streams?.[0]) setRemote(e.streams[0]); });
