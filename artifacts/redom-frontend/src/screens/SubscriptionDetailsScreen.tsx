...
export function SubscriptionDetailsScreen(){
 ...
 return <SafeAreaView style={[styles.root,{backgroundColor:colors.background}]}>
  <View style={[styles.header,{backgroundColor:colors.surface,borderBottomColor:colors.border}]}><Pressable onPress={()=>navigation.goBack()}><Text style={[styles.back,{color:colors.text}]}>‹</Text></Pressable><Text style={[styles.title,{color:colors.text}]}>Subscription details</Text><View style={{width:42}}/></View>
  <ScrollView>
   <View style={[styles.top,{borderBottomColor:colors.border}]}><View style={[styles.logo,{borderColor:colors.border}]}><Text style={styles.logoText}>R</Text></View><View><Text style={[styles.plan,{color:colors.text}]}>{titleForPlan(x.subscriptionType)}</Text><Text style={[styles.muted,{color:colors.textSecondary}]}>Recurring {x.billingCycle} payment</Text></View></View>
   ...
   <Info label="Payment method" value={x.paymentProvider==="google_play"?"In-app purchase":x.paymentProvider==="stripe"?"Stripe":x.paymentProvider==="paypal"?"PayPal":x.paymentProvider||"Not recorded"} colors={colors}/>
   ...
   <Pressable onPress={()=>expired?Alert.alert("Renew subscription","Renewal will continue through a supported payment provider. The payment flow will be shown when renewal is available."):Alert.alert("Manage subscription","Subscription management is handled through the supported payment provider.")} style={[styles.button,{backgroundColor:colors.primary}]}><Text style={styles.buttonText}>{expired?"Renew subscription":"Manage subscription"}</Text></Pressable>
...
const styles=StyleSheet.create({root:{flex:1},header:{height:58,borderBottomWidth:1,flexDirection:"row",alignItems:"center",paddingHorizontal:14},back:{fontSize:40,width:42},title:{fontSize:19,fontWeight:"800",flex:1,textAlign:"center"},top:{padding:20,borderBottomWidth:1,flexDirection:"row",alignItems:"center",gap:14},logo:{width:78,height:78,borderRadius:39,borderWidth:1,alignItems:"center",justifyContent:"center"},logoText:{fontSize:36,color:"#1877F2",fontWeight:"900"},plan:{fontSize:23,fontWeight:"800"},muted:{fontSize:15,lineHeight:22,marginTop:5},info:{paddingHorizontal:30,paddingTop:20},label:{fontSize:14},value:{fontSize:17,marginTop:3},notice:{padding:20,marginTop:24,borderTopWidth:1,borderBottomWidth:1},button:{margin:20,paddingVertical:15,borderRadius:10,alignItems:"center"},buttonText:{color:"#fff",fontSize:16,fontWeight:"800"},help:{textAlign:"center",fontSize:16,fontWeight:"700",marginBottom:30},loader:{marginTop:20}});