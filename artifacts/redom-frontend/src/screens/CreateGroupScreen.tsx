...
export function CreateGroupScreen() {
  ...
  const choosePhoto = async () => {
    try {
      ...
      if (!permission.granted) {
        Alert.alert("Permission needed", "Allow ReDom to access photos to choose a group icon.");
        return;
      }
      ...
      Alert.alert("Group icon", e instanceof Error ? e.message : "The group icon could not be selected.");
    }
  };
  ...
  const chooseTimer = () => {
    Alert.alert(
      ...
      "All new messages in this group will disappear after the selected duration.",
      timerOptions.map((option) => ({
        text: option.value === timer ? `✓ ${option.label}` : option.label,
  ...
  const showPermissions = () => {
    Alert.alert(
      "Group permissions",
      "Members can edit group settings, send new messages, and add other members. Invite links and member approval remain controlled by group admins.",
    );
  };
  ...
  const create = async () => {
    ...
    if (members.some((id) => !/^[0-9a-f]{8}-[0-9a-f-]{27,36}$/i.test(id))) {
      setError("Each member must use a valid ReDom profile ID.");
      return;
    }
    ...
    try {
      ...
      await groupService.updateSettings(result.conversationId, {
        anyoneCanEditInfo: true,
        anyoneCanInvite: true,
        anyoneCanSendMessages: true,
      });
    ...
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        ...
        <Pressable disabled={saving} onPress={() => void create()} style={styles.headerCheck} hitSlop={8}>
          {saving ? <ActivityIndicator color="#FFF" /> : <GroupActionIcon kind="down" size={26} color="#FFF" />}
        </Pressable>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.identityRow}>
          ...
          <Pressable onPress={() => setName((value) => value)} hitSlop={8}>
            <GroupActionIcon kind="emoji" size={27} color="#98A2B3" />
          </Pressable>
        </View>
        ...
        <Pressable style={styles.row} onPress={chooseTimer}>
          <View style={styles.rowText}>
            ...
            <Text style={styles.rowSub}>{timerOptions.find((option) => option.value === timer)?.label || "Off"}</Text>
          </View>
          <GroupActionIcon kind="history" size={30} color="#98A2B3" />
        </Pressable>

        <Pressable style={styles.row} onPress={showPermissions}>
          <Text style={styles.rowTitle}>Group permissions</Text>
          <GroupActionIcon kind="settings" size={30} color="#667085" />
        </Pressable>

        <View style={styles.membersSection}>
          <Text style={styles.memberCount}>Members: {members.length}</Text>
          {members.length === 0 ? (
            <Text style={styles.memberHint}>Add member profile IDs below</Text>
          ) : (
            members.map((id) => (
              <View key={id} style={styles.memberRow}>
                <View style={styles.memberAvatar}>
                  <GroupActionIcon kind="members" size={30} color="#1877F2" />
                </View>
                <Text style={styles.memberId} numberOfLines={1}>{id}</Text>
                <Pressable onPress={() => setMemberIds((value) => value.split(/[\s,]+/).filter((item) => item && item !== id).join(" "))} hitSlop={8}>
                  <GroupActionIcon kind="close" size={22} color="#667085" />
                </Pressable>
              </View>
            ))
          )}
          <TextInput
            ...
            placeholder="Paste profile IDs separated by spaces or commas"
...
const styles = StyleSheet.create({
  ...
  nameBox: { flex: 1, height: 106, borderWidth: 1.5, borderColor: "#98A2B3", borderRadius: 15, justifyContent: "center" },
