...
export function StarsCheckoutScreen(){
 ...
 const[pkg,setPkg]=useState<StarPackage|null>(null);const[countryName,setCountryName]=useState(countryCode);const[methods,setMethods]=useState<SavedPaymentMethod[]>([]);const[selectedMethod,setSelectedMethod]=useState<string|null>(null);
 ...
 useEffect(()=>{(async()=>{try{const[catalog,settings]=await Promise.all([ordersPaymentsService.starsCatalog(countryCode),ordersPaymentsService.getSettings()]);setPkg(catalog.packages.find(x=>x.key===packageKey)||null);setCountryName(catalog.selectedCountry?.name||countryCode);setPinEnabled(Boolean(settings.settings.pin_enabled));try{const pm=await ordersPaymentsService.paymentMethods();setMethods(pm.methods)}catch{setMethods([])}}catch(e){Alert.alert("ReDom Stars checkout",e instanceof Error?e.message:"Unable to load this Stars package.");}finally{setLoading(false)}})()},[countryCode,packageKey]);
 ...
 return <SafeAreaView style={[s.root,{backgroundColor:colors.background}]}>
  ...
  <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
   ...
   {pkg&&<><Text style={[s.price,{color:colors.text}]}>{pkg.localAmountFormatted+" · $"+pkg.usdPrice.toFixed(2)+" USD"}</Text>{pkg.firstPurchaseUsdPrice!=null&&pkg.usdPrice<pkg.regularUsdPrice?<Text style={[s.discount,{color:colors.primary}]}>{("$"+pkg.regularUsdPrice.toFixed(2))+" regular · "+pkg.firstPurchaseDiscountPercent+"% off first purchase"}</Text>:null}</>}
...
const s=StyleSheet.create({root:{flex:1},header:{height:58,borderBottomWidth:1,flexDirection:"row",alignItems:"center",paddingHorizontal:14},title:{fontSize:18,fontWeight:"800",marginLeft:12},content:{padding:20,paddingBottom:40},heading:{fontSize:25,fontWeight:"900"},price:{fontSize:21,fontWeight:"800",marginTop:6},discount:{fontSize:13,fontWeight:"800",marginTop:5},muted:{fontSize:14,lineHeight:20,marginTop:7},label:{fontSize:16,fontWeight:"700",marginTop:22,marginBottom:8},input:{height:54,borderWidth:1,borderRadius:13,paddingHorizontal:15,fontSize:16,marginBottom:10},method:{borderWidth:1,borderRadius:14,padding:14,marginBottom:8},methodTitle:{fontSize:17,fontWeight:"800"},row:{flexDirection:"row",gap:10},half:{flex:1},suggestion:{padding:13,borderWidth:1,borderRadius:10,marginBottom:6},status:{fontSize:13,marginTop:12},button:{height:54,borderRadius:27,alignItems:"center",justifyContent:"center",marginTop:20},buttonText:{color:"#fff",fontSize:17,fontWeight:"800"}});
