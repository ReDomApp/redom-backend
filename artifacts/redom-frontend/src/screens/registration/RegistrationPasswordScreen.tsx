...
type PasswordStrength = { percent: number; label: "Too short" | "Weak" | "Fair" | "Good" | "Strong"; detail: string };

function getPasswordStrength(value: string): PasswordStrength {
  if (!value) return { percent: 0, label: "Too short", detail: "Enter a password using both letters and numbers." };
  const letters = /\p{L}/u.test(value);
  const numbers = /\p{N}/u.test(value);
  const symbols = /[^\p{L}\p{N}]/u.test(value);
  const length = value.length;

  // The score is intentionally visible to the user. Symbols are allowed, but a
  // password must contain at least one letter AND one number. A 12+ character
  // mixed letter/number password reaches 100%; symbols can improve shorter ones.
  let percent = 0;
  if (length >= 6) percent += Math.min(40, 20 + (length - 6) * 4);
  if (letters) percent += 25;
  if (numbers) percent += 25;
  if (symbols) percent += 10;
  percent = Math.min(100, percent);

  const normalized = value.toLowerCase();
  const common = /^(123456|1234567|12345678|password|qwerty|abcdef|abcdefg|111111|000000|letmein)$/i.test(normalized);
  if (common) percent = Math.min(percent, 23);

  if (percent >= 100 && letters && numbers && length >= 12 && !common) {
    return { percent: 100, label: "Strong", detail: "Excellent. This password combines letters and numbers and is long enough for strong account protection." };
  }
  if (!letters || !numbers) {
    return {
      percent,
      label: percent >= 50 ? "Fair" : "Weak",
      detail: !letters && !numbers ? "Add at least one letter and one number." : !letters ? "Add at least one letter. Numbers alone are not accepted." : "Add at least one number. Letters alone are not accepted.",
    };
  }
  if (percent >= 75) return { percent, label: "Good", detail: "Good password. Add more characters to reach 100% and continue." };
  if (percent >= 50) return { percent, label: "Fair", detail: "Fair password. Add more characters, and consider using symbols for additional strength." };
  return { percent, label: "Weak", detail: "Weak password. Use a longer combination of letters and numbers." };
}

export function RegistrationPasswordScreen({ route, navigation }: Props) {
  ...
  const canContinue = strength.percent === 100 && hasLetter && hasNumber && seconds > 0 && !busy && !saved;
  ...
  async function savePassword() {
    ...
    try {
      ...
      await authService.saveRegistrationFlowPassword({ reservationId, flowId, deviceId, password, strength: "strong", rememberLoginInfo: remember });
      setSaved(true);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unable to save your password.");
    } finally { animation.stop(); setBusy(false); }
  }

  const rotation = spin.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "360deg"] });
  const meterWidth = `${strength.percent}%` as `${number}%`;
  ...
  return <View style={styles.root}>
    <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      <Text style={styles.description}>Create a password with letters and numbers. Any character is allowed.</Text>
      ...
      <View style={styles.strengthBox}>
        <View style={styles.strengthHeader}><Text style={styles.strengthTitle}>{strength.label}</Text><Text style={styles.percent}>{strength.percent}%</Text></View>
        <View style={styles.meter}><View style={[styles.meterFill, { width: meterWidth }]}/></View>
        <Text style={styles.strengthBody}>{strength.detail}</Text>
        {password.length > 0 && !hasLetter ? <Text style={styles.error}>At least one letter is required.</Text> : null}
        {password.length > 0 && !hasNumber ? <Text style={styles.error}>At least one number is required.</Text> : null}
      </View>
      ...
      <Text style={styles.rememberHint}>If enabled, your device's supported password manager may offer to save your login information for this device.</Text>
      {error ? <Text style={styles.error}>{error}</Text> : null}
      <Pressable onPress={() => { if (saved) navigation.navigate("RegistrationReview", route.params); else void savePassword(); }} disabled={!canContinue && !saved} style={[styles.button, !canContinue && !saved && styles.disabled]}>
        {busy ? <Animated.View style={[styles.buttonSpinner, { transform: [{ rotate: rotation }] }]}><View style={styles.spinnerArc}/></Animated.View> : <Text style={styles.buttonText}>{saved ? "Continue" : "Continue"}</Text>}
      </Pressable>
      <View style={styles.progress}>{[0,1,2,3,4,5].map(i => <View key={i} style={[styles.dot, i <= (saved ? 5 : 4) && styles.activeDot]}/>)}</View>
...
const styles = StyleSheet.create({
  root:{flex:1,backgroundColor:"#FFF"}, content:{flexGrow:1,width:"100%",maxWidth:430,alignSelf:"center",alignItems:"center",padding:22,paddingTop:36,paddingBottom:30},
  flow:{flexDirection:"row",alignItems:"center",gap:7,marginTop:16}, flowText:{color:BLUE,fontSize:13,fontWeight:"800"}, timer:{color:MUTED,fontSize:12,fontWeight:"700",marginLeft:4},
  title:{color:TEXT,fontSize:30,lineHeight:36,fontWeight:"800",textAlign:"center",marginTop:24}, description:{color:TEXT,fontSize:15,lineHeight:22,textAlign:"center",marginTop:9},
  small:{width:"100%",color:MUTED,fontSize:12.5,marginTop:7}, infoRow:{flexDirection:"row",alignItems:"center",gap:7,marginTop:11,marginBottom:19}, infoLink:{color:TEXT,fontSize:13,fontWeight:"700",textDecorationLine:"underline"},
  field:{width:"100%",height:64,borderWidth:1.5,borderColor:BORDER,borderRadius:13,paddingHorizontal:14,justifyContent:"center",position:"relative"}, fieldFocused:{borderColor:BLUE}, label:{position:"absolute",left:14,color:"#8A8D91",fontSize:16,top:20}, labelFloating:{top:-9,left:11,paddingHorizontal:6,backgroundColor:"#FFF",color:BLUE,fontSize:11,fontWeight:"800"},
  input:{width:"100%",height:34,color:TEXT,fontSize:16,padding:0,paddingRight:38,marginTop:4}, eye:{position:"absolute",right:13,top:21},
  strengthBox:{width:"100%",marginTop:14,padding:14,borderRadius:13,backgroundColor:"#F5F6F7"}, strengthHeader:{flexDirection:"row",alignItems:"center",justifyContent:"space-between"}, strengthTitle:{color:TEXT,fontSize:15,fontWeight:"800"}, percent:{color:BLUE,fontSize:15,fontWeight:"900"},
  meter:{height:8,borderRadius:5,backgroundColor:"#D8DCE1",marginTop:9,overflow:"hidden"}, meterFill:{height:"100%",borderRadius:5,backgroundColor:BLUE,minWidth:0}, strengthBody:{color:MUTED,fontSize:12.5,lineHeight:18,marginTop:8},
  error:{color:ERROR,fontSize:13,lineHeight:19,fontWeight:"600",marginTop:9}, rememberRow:{width:"100%",flexDirection:"row",alignItems:"center",gap:10,marginTop:17}, checkbox:{width:22,height:22,borderRadius:5,borderWidth:1.5,borderColor:"#8A8D91",alignItems:"center",justifyContent:"center"}, checked:{backgroundColor:BLUE,borderColor:BLUE}, check:{color:"#FFF",fontSize:17,fontWeight:"900",lineHeight:20}, rememberText:{color:TEXT,fontSize:14,fontWeight:"600",flex:1}, rememberHint:{width:"100%",color:MUTED,fontSize:11.5,lineHeight:17,marginTop:5,marginLeft:32},
  button:{width:"100%",height:54,borderRadius:13,backgroundColor:BLUE,alignItems:"center",justifyContent:"center",marginTop:21}, buttonText:{color:"#FFF",fontSize:17,fontWeight:"800"}, disabled:{opacity:.5}, buttonSpinner:{width:25,height:25,alignItems:"center",justifyContent:"center"}, spinnerArc:{width:25,height:25,borderRadius:13,borderWidth:3,borderColor:"#FFF",borderRightColor:"transparent",borderBottomColor:"transparent"},
  progress:{flexDirection:"row",justifyContent:"center",gap:8,marginTop:26}, dot:{width:8,height:8,borderRadius:4,backgroundColor:"#D8DCE1"}, activeDot:{width:24,backgroundColor:BLUE}, footer:{alignItems:"center",gap:5,marginTop:17}, copyright:{color:MUTED,fontSize:12},
