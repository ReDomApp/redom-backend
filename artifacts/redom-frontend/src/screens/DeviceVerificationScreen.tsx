...
export function DeviceVerificationScreen({ route, navigation }: Props) {
  ...
  const footer = <View style={styles.footer}><SecurityShield width={18} height={18} /><Text style={styles.footerText}>ReDom Platforms, Inc.</Text></View>;
    ...
    <ReDomScreen footer={footer}>
      <View style={styles.content}>
        ...
        <Pressable onPress={() => inputRef.current?.focus()} disabled={loading} style={styles.codeRow}>
          {Array.from({ length: CODE_LENGTH }).map((_, index) => <View key={index} style={[styles.codeBox, index === code.length && styles.codeBoxActive]}><Text style={styles.codeDigit}>{code[index] || ""}</Text></View>)}
        </Pressable>
        <TextInput ref={inputRef} value={code} onChangeText={(value) => { setError(null); setCode(value.replace(/\D/g, "").slice(0, CODE_LENGTH)); }} keyboardType="number-pad" textContentType="oneTimeCode" autoComplete="sms-otp" maxLength={CODE_LENGTH} editable={!loading} style={styles.hiddenInput} accessibilityLabel="Verification code" />
        {error ? <Text accessibilityRole="alert" style={styles.error}>{error}</Text> : null}
        <Pressable onPress={() => void submit()} disabled={loading || code.length !== CODE_LENGTH} style={[styles.primaryButton, (loading || code.length !== CODE_LENGTH) && styles.disabled]}>{loading ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.primaryText}>Verify device</Text>}</Pressable>
        <Text style={styles.expiry}>{secondsLeft > 0 ? `Code expires in ${minutes}:${seconds}` : "This code has expired."}</Text>
        <Pressable onPress={() => navigation.goBack()} disabled={loading} style={styles.backButton}><Text style={styles.backText}>Back to Login</Text></Pressable>
      </View>
    </ReDomScreen>
  );
}
...
const styles = StyleSheet.create({
  content: { flex: 1, width: "100%", justifyContent: "center", alignItems: "center", paddingVertical: 8 },
  iconCircle: { width: 68, height: 68, borderRadius: 34, backgroundColor: "#EAF2FF", alignItems: "center", justifyContent: "center", marginBottom: 20 },
  title: { textAlign: "center", fontSize: 25, fontWeight: "800", color: "#1C1E21", marginBottom: 10 },
  subtitle: { width: "100%", textAlign: "center", fontSize: 15, lineHeight: 23, color: "#65676B", marginBottom: 28 },
  codeRow: { width: "100%", flexDirection: "row", justifyContent: "center", gap: 8, marginBottom: 8 },
  codeBox: { flex: 1, maxWidth: 52, minWidth: 40, height: 52, borderWidth: 1.5, borderColor: "#CCD0D5", borderRadius: 10, alignItems: "center", justifyContent: "center", backgroundColor: "#FFFFFF" },
  ...
  primaryButton: { width: "100%", height: 54, borderRadius: 12, backgroundColor: BLUE, alignItems: "center", justifyContent: "center", marginTop: 20 },
  ...
  footer: { width: "100%", flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7 },
  footerText: { color: "#65676B", fontSize: 12 },
});
