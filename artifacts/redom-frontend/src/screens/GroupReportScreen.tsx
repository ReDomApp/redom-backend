...
export function GroupReportScreen({ route, navigation }: Props) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await groupReportService.submit(route.params.conversationId, { reason, details: details.trim() || undefined, exitAfterReport });
      if (exitAfterReport) {
        try { await chatInfoService.clearChat(route.params.conversationId); } catch { /* the backend has already exited the member */ }
      }
      Alert.alert("Report submitted", `ReDom AI Moderation checked ${result.evidenceCount} recent message(s). Status: ${result.status.replace(/_/g, " ")}.${result.emailChecked ? " A report result email was checked for your connected verified email." : " No connected verified email is available for report notifications."}`, [{ text: "Done", onPress: () => navigation.goBack() }]);
    } catch (error) {
  ...
  return <SafeAreaView style={s.root}>
    <View style={s.header}><Pressable onPress={() => navigation.goBack()}><GroupActionIcon kind="back" size={28} color="#111827" /></Pressable><Text style={s.title}>Report group</Text><View style={{ width: 28 }} /></View>
