...
export function ProfileScreen({ navigation, route }: Props) {
  ...
  const refreshProfile = useCallback(async () => {
    ...
    try {
      ...
      setPromptIndex((current) => (current + 1) % PROFILE_PROMPTS.length);
    } catch {
  ...
  const add = async (suggestion: ProfileSuggestion) => {
    ...
    try {
      ...
      Alert.alert(
        result.success ? "Friend request sent" : "Couldn't add this person",
        result.success ? "Your friend request was sent." : (result.reason || "We couldn't send the request right now."),
      );
    } finally {
      setAdding(null);
    }
  ...
  const copyProfileIdentifier = async () => {
  ...
  return (
    <SafeAreaView style={ui.root}>
      <View style={ui.nav}>
        <Pressable style={ui.navItem} onPress={() => navigation.navigate("HomeFeed")}>
          <HomeIcon width={ui.icon} height={ui.icon} />
        </Pressable>
        <Pressable style={ui.navItem}>
          <VideoIcon width={ui.icon} height={ui.icon} />
        </Pressable>
        <Pressable style={ui.navItem}>
          <MarketplaceIcon width={ui.icon} height={ui.icon} />
        </Pressable>
        <Pressable style={ui.navItem}>
          <NotificationsIcon width={ui.icon} height={ui.icon} />
        </Pressable>
        <Pressable
      ...
      >
        <View style={ui.coverWrap}>
          {p?.coverPhoto ? (
            <ImageBackground source={{ uri: p.coverPhoto }} style={ui.cover} resizeMode="cover">
              <View style={ui.coverShade} />
            </ImageBackground>
          ) : (
            <View style={ui.cover}>
              <View style={ui.coverShade} />
          ...
          <View style={ui.coverTools}>
            ...
            <Pressable accessibilityLabel="Search profile" onPress={() => setSearchOpen(true)}>
              <CoverSearchIcon width={ui.coverToolIcon} height={ui.coverToolIcon} />
            </Pressable>
            <Pressable
          ...
          <Pressable style={ui.songBubble} accessibilityLabel="Profile prompt">
        ...
        <View style={ui.identityBlock}>
          <View style={ui.identityTop}>
            <View style={ui.identityText}>
              <View style={ui.nameRow}>
                <Text style={ui.name} numberOfLines={2}>{name}</Text>
                {p?.verified ? <Text style={ui.verifiedBadge}>✓</Text> : null}
              </View>
              <Text style={ui.username}>{p?.username ? `@${p.username}` : ""}</Text>
              <Text style={ui.countLine}>{p?.friendCount ?? 0} Friends <Text style={ui.dot}>•</Text> {p?.postCount ?? 0} posts</Text>
            </View>
            <Pressable style={ui.dropdown} accessibilityLabel="Additional profile information">
              <DropdownIcon width={ui.dropdownSize} height={ui.dropdownSize} />
            </Pressable>
          </View>

          {p?.location ? (
            <View style={ui.locationLine}>
              <LocationIcon width={ui.detailIcon} height={ui.detailIcon} />
              <Text style={ui.locationText}>{p.location}</Text>
            </View>
          ) : null}
            ...
            {isOwner ? (
              <>
                <Pressable style={ui.primaryButton} accessibilityLabel="Add to story">
                  ...
                  <Text style={ui.primaryText}>Add to story</Text>
                </Pressable>
                <Pressable style={ui.secondaryButton} accessibilityLabel="Edit profile">
              ...
              <>
                <Pressable style={ui.primaryButton} accessibilityLabel={`Add ${name} as a friend`}>
                  <Text style={ui.primaryText}>Add Friend</Text>
                </Pressable>
                <Pressable style={ui.secondaryButton} accessibilityLabel={`Message ${name}`}>
                  <Text style={ui.secondaryText}>Message</Text>
                </Pressable>
              </>
            )}
          </View>
        ...
        {isOwner && p && p.suggestions.length > 0 ? (
          <View style={ui.suggestionSection}>
            <View style={ui.sectionTitleRow}>
              ...
              <Pressable accessibilityLabel="Dismiss suggestions">
                <Text style={ui.closeSuggestion}>×</Text>
              </Pressable>
            </View>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={ui.suggestionRail}>
              {p.suggestions.map((suggestion) => (
                <View key={suggestion.userId} style={ui.suggestionCard}>
                  <View style={ui.suggestionAvatarWrap}>
                    <Avatar uri={suggestion.profilePhoto} size={ui.suggestionAvatar} />
                  </View>
                  <Text style={ui.suggestionName} numberOfLines={1}>{suggestion.firstName} {suggestion.lastName}</Text>
                  <Pressable style={ui.addButton} onPress={() => void add(suggestion)} disabled={adding === suggestion.userId}>
                    <Text style={ui.addText}>{adding === suggestion.userId ? "Adding..." : "Add"}</Text>
                  </Pressable>
                </View>
              ))}
            </ScrollView>
            <Pressable style={ui.addAll} onPress={() => p.suggestions.forEach((suggestion) => void add(suggestion))}>
              <Text style={ui.addAllText}>Add All</Text>
            </Pressable>
          </View>
        ) : null}

        <View style={ui.tabs}>
          {(["All", "Reels", "Photos", "Events"] as const).map((item) => (
            <Pressable key={item} style={[ui.tab, tab === item && ui.tabActive]} onPress={() => setTab(item)}>
              <Text style={[ui.tabText, tab === item && ui.tabTextActive]}>{item}</Text>
            </Pressable>
          ))}
        </View>

        {tab === "Reels" ? (
          <View style={ui.mediaSection}>
            <Text style={ui.sectionTitle}>Reels</Text>
            {p?.reels.length ? (
              <View style={ui.reelGrid}>
                {p.reels.map((reel) => (
                  <View key={reel.id} style={ui.reelCard}>
                    {reel.thumbnail ? <Image source={{ uri: reel.thumbnail }} style={ui.mediaImage} /> : <View style={ui.mediaImage} />}
                    <View style={ui.viewOverlay}>
                      <EyeIcon width={ui.viewIcon} height={ui.viewIcon} />
                      <Text style={ui.viewText}>{compact(reel.viewCount)}</Text>
                    </View>
                  </View>
                ))}
              </View>
        ...
        {tab === "Events" ? (
          <View style={ui.mediaSection}>
            <Text style={ui.sectionTitle}>Events</Text>
            <Text style={ui.emptyText}>No events to show.</Text>
          </View>
        ) : null}
            ...
            <View style={ui.detailsSection}>
              ...
              {p?.location ? (
                <View style={ui.detailRow}>
                  <LocationIcon width={ui.detailIcon} height={ui.detailIcon} />
                  <Text style={ui.detailText}>{p.location}</Text>
                </View>
              ) : null}
              {p?.birthday ? (
                <View style={ui.detailRow}>
                  <CalendarIcon width={ui.detailIcon} height={ui.detailIcon} />
                  <Text style={ui.detailText}>{p.birthday}</Text>
                </View>
              ) : null}
            </View>

            <View style={ui.detailsSection}>
              <Text style={ui.sectionTitle}>ReDom</Text>
              <View style={ui.detailRow}>
                <ReDomRecordIcon width={ui.detailIcon} height={ui.detailIcon} />
                <View style={ui.redomValue}>
                  <Text style={ui.detailText}>Joined ReDom</Text>
                  {p?.joinedAt ? <Text style={ui.subDetail}>{p.joinedAt}</Text> : null}
                  {p?.joinedCountry ? <Text style={ui.subDetail}>{p.joinedCountry}</Text> : null}
                </View>
              </View>
            </View>

            <View style={ui.friendsSection}>
              <View style={ui.sectionTitleRow}>
                <Text style={ui.sectionTitle}>Friends</Text>
                <Pressable accessibilityLabel="See all friends"><Text style={ui.seeAll}>See all</Text></Pressable>
              </View>
              {p?.friends.length ? (
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={ui.friendRail}>
                  {p.friends.slice(0, 12).map((friend) => (
                    <View key={friend.userId} style={ui.friendCard}>
                      <Avatar uri={friend.profilePhoto} size={ui.friendAvatar} />
                      <Text style={ui.friendName} numberOfLines={1}>{friend.firstName}</Text>
                    </View>
                  ))}
                </ScrollView>
              ) : null}
            ...
            {isOwner ? (
              <>
                <View style={ui.postsHeader}>
                  ...
                  <Pressable onPress={() => setFilterOpen(true)}><Text style={ui.seeAll}>Filters</Text></Pressable>
                </View>
                <View style={ui.composer}>
                  <Avatar uri={p?.profilePhoto || user?.profilePhoto} size={ui.composerAvatar} />
                  <Pressable style={ui.composerBox} accessibilityLabel="Create a post">
                    <Text style={ui.composerText}>What's on your mind?</Text>
                  </Pressable>
                  <Pressable accessibilityLabel="Choose photos">
                    <GalleryIcon width={ui.buttonIcon} height={ui.buttonIcon} />
                  </Pressable>
                </View>
                <View style={ui.creationRow}>
                  <Pressable style={ui.creationButton} accessibilityLabel="Create reel">
                    <ReelIcon width={ui.buttonIcon} height={ui.buttonIcon} />
                    <Text style={ui.creationText}>Reel</Text>
                  </Pressable>
                  <Pressable style={ui.creationButton} accessibilityLabel="Go live">
                    <LiveIcon width={ui.buttonIcon} height={ui.buttonIcon} />
                    <Text style={ui.creationText}>Live</Text>
                  </Pressable>
                </View>
              </>
            ) : null}

            {p?.posts.length ? p.posts.map((post) => (
              <View key={post.id} style={ui.post}>
                <View style={ui.postHead}>
                  <Avatar uri={p.profilePhoto} size={ui.postAvatar} />
                  <View>
                    <Text style={ui.postName}>{name}</Text>
                    <Text style={ui.postTime}>{new Date(post.publishedAt).toLocaleDateString()}</Text>
                  </View>
                </View>
                {post.content ? <Text style={ui.postContent}>{post.content}</Text> : null}
                {post.thumbnail ? <Image source={{ uri: post.thumbnail }} style={ui.postMedia} /> : null}
              </View>
            )) : null}
          </>
        ) : null}
      ...
      <Modal visible={searchOpen} transparent animationType="fade" onRequestClose={() => setSearchOpen(false)}>
        <View style={ui.modalBackdrop}>
          <View style={ui.searchPanel}>
            <View style={ui.modalHeader}>
              <Text style={ui.modalTitle}>Search {firstName || "profile"}</Text>
              <Pressable onPress={() => setSearchOpen(false)}><Text style={ui.modalClose}>×</Text></Pressable>
            </View>
            <TextInput value={searchText} onChangeText={setSearchText} placeholder="Search this profile" placeholderTextColor="#65676B" style={ui.searchInput} autoFocus />
            <Text style={ui.searchHint}>Search posts, media or tagged items within this profile.</Text>
            <View style={ui.dateRow}>
              {["Most recent", "Last Yr", "Last Month", "Today", "Select Date"].map((label) => <Pressable key={label} style={ui.dateChip}><Text style={ui.dateChipText}>{label}</Text></Pressable>)}
            </View>
            {searchText.trim() ? <Text style={ui.emptyText}>Search results will appear here.</Text> : null}
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={filterOpen} transparent animationType="slide" onRequestClose={() => setFilterOpen(false)}>
        <View style={ui.modalBackdrop}>
          <View style={ui.filterPanel}>
            <View style={ui.modalHeader}><Text style={ui.modalTitle}>Filter posts</Text><Pressable onPress={() => setFilterOpen(false)}><Text style={ui.modalClose}>×</Text></Pressable></View>
            {["Year", "Posted By", "Privacy status"].map((label) => <View key={label} style={ui.disabledRow}><Text style={ui.disabledText}>{label}</Text></View>)}
          </View>
        </View>
      </Modal>
    </SafeAreaView>
...
function ProfileSheet({
  ...
  const rows = owner ? [
    ...
    [SettingsIcon, "Turn on Business Mode"],
    [SettingsIcon, "Turn On Creator Mode"],
  ] as const : visitor ? [
    [SettingsIcon, "Block User"],
    [ReportIcon, "Report Profile"],
    [ShareIcon, "Share Profile"],
    [FollowIcon, "Unfollow"],
  ] as const : cover ? [
    [CoverCameraIcon, "View Cover Photo"],
    [CoverCameraIcon, "Upload New Cover Photo"],
  ] as const : [
    [CameraIcon, "See Profile Picture"],
    [CameraIcon, "Choose Profile Picture"],
    [CameraIcon, "Create Avatar Profile Picture"],
  ] as const;
        ...
        <View style={ui.sheet}>
          ...
          <View style={ui.sheetHeader}>
            <Text style={ui.sheetTitle}>
              {owner ? "Profile management" : visitor ? "Manage profile" : cover ? "Cover photo" : "Profile picture"}
            </Text>
          ...
          {!cover && !avatar ? (
            <>
              ...
              <View style={ui.linkIntro}>
                <LinkIcon width={ui.sheetIcon} height={ui.sheetIcon} />
                <View style={ui.linkCopy}>
                  <Text style={ui.sheetRowText}>{owner ? `Copy ReDom Profile Link for ${firstName || name}?` : `Copy ${firstName || name}'s personalized link on ReDom?`}</Text>
                  <Text style={ui.linkDescription}>This is {name}'s personalized link on ReDom.</Text>
                </View>
              </View>
              <Pressable style={ui.copyButton} onPress={() => void onCopy()}>
                <LinkIcon width={ui.buttonIcon} height={ui.buttonIcon} />
                <Text style={ui.copyButtonText}>Copy Profile Link</Text>
              </Pressable>
            </>
          ) : null}
        </View>
...
function makeStyles(width: number) {
  ...
  return StyleSheet.create({
    ...
    menuButton: { position: "absolute", left: n(10), top: n(14), width: n(42), height: n(42), alignItems: "center", justifyContent: "center" },
    coverTools: { position: "absolute", right: n(10), top: n(14), flexDirection: "row", alignItems: "center", gap: n(10) },
    coverToolIcon: n(30), songBubble: { position: "absolute", left: n(96), top: n(116), backgroundColor: "#fff", borderRadius: n(22), paddingHorizontal: n(14), paddingVertical: n(9), elevation: 2, maxWidth: width - n(150) },
    songText: { fontSize: n(14), color: "#65676B" }, cameraIcon: n(28),
    ...
    profileAvatar: avatar, profileCamera: { position: "absolute", right: -n(3), bottom: 0, width: n(40), height: n(40), borderRadius: n(20), backgroundColor: "#E4E6EB", alignItems: "center", justifyContent: "center" },
    identityBlock: { paddingHorizontal: horizontal, paddingBottom: n(12) }, identityTop: { flexDirection: "row", alignItems: "flex-start" },
    identityText: { marginLeft: avatar + n(12), flex: 1, minWidth: 0 }, nameRow: { flexDirection: "row", alignItems: "center", gap: n(5) },
    name: { fontSize: n(23), fontWeight: "800", color: "#050505", lineHeight: n(27), flexShrink: 1 },
    verifiedBadge: { backgroundColor: "#1877F2", color: "#fff", width: n(19), height: n(19), borderRadius: n(10), textAlign: "center", lineHeight: n(19), fontWeight: "800", fontSize: n(12) },
    username: { fontSize: n(13), color: "#65676B", marginTop: n(1) }, countLine: { fontSize: n(14), color: "#65676B", marginTop: n(2) }, dot: { color: "#65676B" },
    dropdown: { width: n(44), height: n(44), alignItems: "center", justifyContent: "center", marginTop: n(4) }, dropdownSize: n(40),
    ...
    mediaSection: { padding: horizontal, gap: n(12) }, reelGrid: { flexDirection: "row", flexWrap: "wrap", gap: n(4) }, reelCard: { width: (width - horizontal * 2 - n(8)) / 3, aspectRatio: 0.72, backgroundColor: "#E4E6EB", overflow: "hidden" }, mediaImage: { width: "100%", height: "100%", backgroundColor: "#E4E6EB" }, viewOverlay: { position: "absolute", left: n(6), bottom: n(6), flexDirection: "row", alignItems: "center", gap: n(3) }, viewIcon: n(16), viewText: { color: "#fff", fontWeight: "700", fontSize: n(12) }, photoGrid: { flexDirection: "row", flexWrap: "wrap", gap: n(4) }, photoCard: { width: (width - horizontal * 2 - n(8)) / 3, height: (width - horizontal * 2 - n(8)) / 3, backgroundColor: "#E4E6EB" },
    emptyText: { color: "#65676B", fontSize: n(15), paddingVertical: n(10) }, detailsSection: { paddingHorizontal: horizontal, paddingTop: n(20), gap: n(14) }, editIcon: n(25), detailRow: { flexDirection: "row", alignItems: "center", gap: n(13) }, detailText: { fontSize: n(17), fontWeight: "600", color: "#050505" }, subDetail: { fontSize: n(14), color: "#65676B", marginTop: n(2) }, redomValue: { flex: 1 },
    friendsSection: { paddingHorizontal: horizontal, paddingTop: n(24) }, seeAll: { color: "#1877F2", fontSize: n(16), fontWeight: "600" }, friendRail: { gap: n(12), paddingVertical: n(12) }, friendCard: { width: n(72), alignItems: "center" }, friendAvatar: n(62), friendName: { marginTop: n(5), fontSize: n(13), fontWeight: "600", maxWidth: n(70), textAlign: "center" },
