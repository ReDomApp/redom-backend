  ...
  return (
    <SafeAreaView style={ui.root}>
      <View style={ui.nav}>
        ...
        <Pressable style={[ui.navItem, ui.activeNav]} onPress={() => (canEdit ? void refreshProfile() : navigation.navigate("Profile"))} accessibilityLabel={canEdit ? "Refresh current profile" : "Open my profile"}>
      ...
      {visitorPreview ? (
        <View style={ui.previewBanner}>
          ...
          <Pressable onPress={leaveVisitorPreview} style={ui.previewDone}><Text style={ui.previewDoneText}>Exit</Text></Pressable>
        </View>
      ) : null}
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={ui.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshProfile} />}>
        <View style={ui.coverWrap}>
          ...
          <View style={ui.avatarPosition}>
            <View style={ui.avatarRing}><Avatar uri={p?.profilePhoto || user?.profilePhoto} size={ui.profileAvatar} /></View>
            {canEdit ? <Pressable style={ui.profileCamera} onPress={() => setSheet("avatar")} accessibilityLabel="Change profile picture"><CameraIcon width={ui.cameraIcon} height={ui.cameraIcon} /></Pressable> : null}
          </View>
        </View>
        ...
        <View style={ui.identityBlock}>
          <View style={ui.identityTop}>
            <View style={ui.identityText}>
              <View style={ui.nameRow}><Text style={ui.name} numberOfLines={2}>{name}</Text>{p?.verified ? <Text style={ui.verifiedBadge}>✓</Text> : null}</View>
            ...
            <Pressable style={ui.dropdown} accessibilityLabel="Additional profile information"><DropdownIcon width={ui.dropdownSize} height={ui.dropdownSize} /></Pressable>
          </View>
        ...
        {canEdit && p && p.suggestions.length > 0 ? <View style={ui.suggestionSection}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={ui.suggestionRail}>{p.suggestions.map((suggestion) => <View key={suggestion.userId} style={ui.suggestionCard}><View style={ui.suggestionAvatarWrap}><Avatar uri={suggestion.profilePhoto} size={ui.suggestionAvatar} /></View><Text style={ui.suggestionName} numberOfLines={1}>{suggestion.firstName} {suggestion.lastName}</Text><Pressable style={ui.addButton} onPress={() => void add(suggestion)} disabled={adding === suggestion.userId}><Text style={ui.addText}>{adding === suggestion.userId ? "Adding..." : "Add"}</Text></Pressable></View>)}</ScrollView>
...
function ProfileSheet({kind,name,firstName,visitorPreview,onClose,onCopy,onViewAsVisitor,ui}:{kind:SheetKind;name:string;firstName:string;visitorPreview:boolean;onClose:()=>void;onCopy:()=>Promise<void>;onViewAsVisitor?:()=>void;ui:ReturnType<typeof makeStyles>}) {
  ...
  const rows = owner ? [
    ...
    [SettingsIcon, "Block User"], [ReportIcon, "Report Profile"], [ShareIcon, "Share Profile"], [FollowIcon, "Unfollow"],
  ] as const : cover ? [[CoverCameraIcon, "View Cover Photo"], [CoverCameraIcon, "Upload New Cover Photo"]] as const : [[CameraIcon, "See Profile Picture"], [CameraIcon, "Choose Profile Picture"], [CameraIcon, "Create Avatar Profile Picture"]] as const;
