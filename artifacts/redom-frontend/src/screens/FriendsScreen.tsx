...
export function FriendsScreen(){
 ...
 const load=useCallback(async(refresh=false)=>{
   ...
   try{
     const result=tab==="friends"?await friendsService.list(query.trim()||undefined):tab==="following"?await friendsService.following():tab==="suggested"?await friendsService.suggested(query.trim()||undefined):await friendsService.common();
     setPeople(result.people??[]);
     if(tab==="friends"){const req=await friendsService.requests();setIncoming(req.incoming??[]);const ids:Record<string,string>={};for(const p of req.incoming??[])if(p.incomingRequestId)ids[p.userId]=p.incomingRequestId;setRequestIds(ids);}
 ...
 const tabs=useMemo(()=>[["friends","Friends"],["following","Following"],["suggested","Suggested"],["common","Things in common"]] as const,[]);
