...
const BLUE = "#1877F2";
const CODE_LENGTH = 6;
type Props = NativeStackScreenProps<RootStackParamList, "LoginTwoFactor">;

export function LoginTwoFactorScreen({ route, navigation }: Props) {
  const { verifyLoginTwoFactor } = useAuthContext();
  const { uiMessage, t } = useLanguage();
  const { challengeId, channel, maskedTarget, expiresAt } = route.params;
  ...
  useEffect(() => { const timer = setInterval(() => setSecondsLeft(v => Math.max(0, v - 1)), 1000); return () => clearInterval(timer); }, []);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const result = await verifyLoginTwoFactor({ challengeId, code, deviceId, deviceName: Platform.OS === "ios" ? "iPhone" : "Android Device", deviceType: "mobile", platform: Platform.OS, loginSource: "mobile", appVersion: "1.0.0" });
      if (!result.success) setError(result.message || "Invalid verification code.");
    } catch (err) { setError(err instanceof Error ? err.message : "Two-factor authentication failed."); }
  ...
  return <ReDomScreen footer={footer}><View style={styles.content}>
    ...
    <Text style={styles.subtitle}>Enter the 6-digit code sent by {channelLabel} to {maskedTarget}.</Text>
    ...
    <TextInput ref={inputRef} value={code} onChangeText={value => { setError(null); setCode(value.replace(/\D/g, "").slice(0, CODE_LENGTH)); }} keyboardType="number-pad" textContentType="oneTimeCode" autoComplete="sms-otp" maxLength={CODE_LENGTH} editable={!loading} style={styles.hiddenInput} />
    {error ? <Text accessibilityRole="alert" style={styles.error}>{error}</Text> : null}
    <Pressable onPress={() => void submit()} disabled={loading || code.length !== CODE_LENGTH} style={[styles.primaryButton, (loading || code.length !== CODE_LENGTH) && styles.disabled]}>{loading ? <ActivityIndicator size="small" color="#FFFFFF" /> : <Text style={styles.primaryText}>Verify</Text>}</Pressable>
    <Text style={styles.expiry}>{secondsLeft > 0 ? `Code expires in ${minutes}:${seconds}` : "Code expired"}</Text>
    <Pressable onPress={() => navigation.goBack()} disabled={loading} style={styles.backButton}><Text style={styles.backText}>Back to verification</Text></Pressable>
  </View></ReDomScreen>;
}
