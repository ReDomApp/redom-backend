...
export function HomeFeedScreenV2() {
  ...
  const removeHidden = (id: string) => setPosts((current) => current.filter((post) => post.id !== id));
  const restorePost = (postId: string) => setPosts((current) => current.map((post) => post.id === postId ? post : post));

  return <SafeAreaView style={styles.root}>
    <View style={styles.topHeader}>
      <ReDomLogo width={116} height={34} />
      <View style={styles.topActions}>
        <Pressable style={styles.topIcon}><SearchIcon width={23} height={23} /></Pressable>
        <Pressable style={styles.topIcon}><MessengerIcon width={24} height={24} /></Pressable>
        <Pressable style={styles.topIcon} onPress={() => setMenuOpen(true)}><MenuIcon width={24} height={24} /></Pressable>
      </View>
    </View>
    <View style={styles.navigationBar}>
      <Pressable style={[styles.navItem, styles.activeNav]} onPress={() => { scrollRef.current?.scrollTo({ y: 0, animated: true }); void refreshFeed(); }}><HomeIcon width={27} height={27} /></Pressable>
      ...
      <Pressable style={styles.navItem}><Avatar size={31} /></Pressable>
    </View>
    ...
    <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshFeed} />} contentContainerStyle={styles.feed}>
      <View style={styles.composer}>
        <Avatar size={42} />
        <View style={styles.composerInput}><Text style={styles.composerHint}>What's on your mind{user?.firstName ? `, ${user.firstName}` : ""}?</Text></View>
        <Pressable><AddMediaIcon width={27} height={27} /></Pressable>
      </View>
      <View style={styles.quickActions}>
        <Pressable style={styles.quick}><CreateIcon width={24} height={24} /><Text>Create post</Text></Pressable>
        <Pressable style={styles.quick}><VideoIcon width={24} height={24} /><Text>Photo/video</Text></Pressable>
        <Pressable style={styles.quick}><UploadStoryIcon width={24} height={24} /><Text>Story</Text></Pressable>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.rail}>
        <Pressable style={styles.story}><Avatar size={70} /><Text style={styles.storyName}>Create story</Text></Pressable>
        {stories.slice(0, 12).map((story) => <Pressable key={story.id} style={styles.story}><View style={styles.storyRing}><Avatar uri={story.profilePhoto} size={64} /></View><Text style={styles.storyName} numberOfLines={1}>{story.firstName}</Text></Pressable>)}
      </ScrollView>
      ...
      {posts.map((post) => <PostCardV2 key={post.id} post={post} onHidden={removeHidden} onUnhidden={restorePost} />)}
...
const styles = StyleSheet.create({
  ...
  topHeader: { height: 58, paddingHorizontal: 10, backgroundColor: "#FFF", flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  topActions: { flexDirection: "row", gap: 8 },
  topIcon: { width: 38, height: 38, borderRadius: 19, backgroundColor: "#E4E6EB", alignItems: "center", justifyContent: "center" },
  navigationBar: { height: 52, backgroundColor: "#FFF", flexDirection: "row", justifyContent: "space-around", alignItems: "center", borderBottomWidth: 1, borderBottomColor: "#E4E6EB" },
  navItem: { width: 58, height: 46, alignItems: "center", justifyContent: "center" },
  activeNav: { borderBottomWidth: 3, borderBottomColor: "#1877F2" },
  feed: { paddingBottom: 24 },
  composer: { backgroundColor: "#FFF", padding: 10, flexDirection: "row", alignItems: "center", gap: 9 },
  composerInput: { flex: 1, backgroundColor: "#F0F2F5", borderRadius: 22, paddingHorizontal: 15, paddingVertical: 11 },
  composerHint: { color: "#65676B", fontSize: 15 },
  quickActions: { backgroundColor: "#FFF", borderTopWidth: 1, borderTopColor: "#E4E6EB", borderBottomWidth: 1, borderBottomColor: "#E4E6EB", height: 48, flexDirection: "row", justifyContent: "space-around", alignItems: "center" },
  quick: { flexDirection: "row", alignItems: "center", gap: 6 },
  sectionHeader: { paddingHorizontal: 12, paddingTop: 15, paddingBottom: 8, backgroundColor: "#F0F2F5", flexDirection: "row", justifyContent: "space-between" },
  sectionTitle: { fontSize: 19, fontWeight: "700" },
  seeAll: { color: "#1877F2", fontSize: 14, fontWeight: "600" },
  rail: { paddingHorizontal: 10, gap: 10, backgroundColor: "#F0F2F5" },
  story: { width: 78, alignItems: "center", paddingBottom: 5 },
  storyRing: { borderWidth: 3, borderColor: "#1877F2", borderRadius: 38, padding: 2 },
  storyName: { fontSize: 12, marginTop: 5, maxWidth: 75, textAlign: "center" },
  suggestion: { width: 120, backgroundColor: "#FFF", borderRadius: 10, padding: 8, alignItems: "center", marginBottom: 5 },
  suggestionName: { fontWeight: "600", marginTop: 5, maxWidth: 108 },
  handle: { color: "#65676B", fontSize: 12, marginTop: 2 },
  ...
  logout: { fontSize: 16, fontWeight: "700" },
});
