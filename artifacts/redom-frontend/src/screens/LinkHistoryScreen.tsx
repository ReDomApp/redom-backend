...
export function LinkHistoryScreen() {
  ...
  return <SafeAreaView style={[styles.root, { backgroundColor: colors.background }]}>
    <View style={[styles.header, { backgroundColor: colors.surface, borderBottomColor: colors.border }]}>
      ...
      <Pressable onPress={clear} disabled={!items.length}><Text style={[styles.clear, { color: items.length ? colors.primary : colors.textSecondary }]}>Clear</Text></Pressable>
    </View>
    <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }} tintColor={colors.primary} />}>
      <Text style={[styles.intro, { color: colors.textSecondary }]}>Links you open from ReDom can appear here. Your history is private to your account.</Text>
      ...
      {loading ? <View style={styles.center}><ActivityIndicator color={colors.primary} /></View> : items.length ? items.map(item =>
        <Pressable key={item.id} style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]} onPress={() => void openExternalLink(item.url, item.title ?? item.domain, "link-history")} onLongPress={() => remove(item.id)}>
          ...
          <View style={styles.body}><Text numberOfLines={1} style={[styles.domain, { color: colors.text }]}>{item.domain}</Text><Text numberOfLines={2} style={[styles.url, { color: colors.textSecondary }]}>{item.title || item.url}</Text><Text style={[styles.time, { color: colors.textSecondary }]}>{new Date(item.openedAt).toLocaleString()}</Text></View>
          <Text style={[styles.more, { color: colors.textSecondary }]}>⋯</Text>
        </Pressable>
      ) : <View style={[styles.empty, { backgroundColor: colors.surface }]}><Text style={[styles.emptyTitle, { color: colors.text }]}>No links yet</Text><Text style={[styles.emptyText, { color: colors.textSecondary }]}>Links you open from ReDom will be saved here.</Text></View>}
      <Pressable onPress={() => navigation.navigate("Policy", { slug: "privacy" })} style={[styles.policy, { backgroundColor: colors.surface }]}><Text style={[styles.policyText, { color: colors.text }]}>Link History Policy</Text><Text style={[styles.chevron, { color: colors.textSecondary }]}>›</Text></Pressable>
    </ScrollView>
  </SafeAreaView>;
}
const styles = StyleSheet.create({ root:{flex:1}, header:{height:58,borderBottomWidth:1,flexDirection:"row",alignItems:"center",justifyContent:"space-between",paddingHorizontal:12}, back:{fontSize:38}, title:{fontSize:19,fontWeight:"800"}, clear:{fontSize:14,fontWeight:"700"}, content:{padding:12,paddingBottom:40},center:{minHeight:180,alignItems:"center",justifyContent:"center"}, intro:{fontSize:14,lineHeight:21,marginBottom:12}, card:{minHeight:78,borderWidth:1,borderRadius:12,padding:12,marginBottom:8,flexDirection:"row",alignItems:"center"}, icon:{width:40,height:40,borderRadius:20,alignItems:"center",justifyContent:"center"}, body:{flex:1,marginLeft:8},domain:{fontSize:15,fontWeight:"800"},url:{fontSize:13,marginTop:3},time:{fontSize:11,marginTop:5},more:{fontSize:22,paddingHorizontal:4},error:{padding:12,borderRadius:10,marginBottom:10},empty:{padding:22,borderRadius:12,alignItems:"center"},emptyTitle:{fontSize:17,fontWeight:"800"},emptyText:{fontSize:14,textAlign:"center",marginTop:6},policy:{marginTop:14,minHeight:54,borderRadius:12,paddingHorizontal:15,flexDirection:"row",alignItems:"center",justifyContent:"space-between"},policyText:{fontSize:15,fontWeight:"700"},chevron:{fontSize:28}});
