...
export function SubscriptionsScreen(){
 ...
 return <SafeAreaView style={[styles.root,{backgroundColor:colors.background}]}>
  <View style={[styles.header,{backgroundColor:colors.surface,borderBottomColor:colors.border}]}><Pressable onPress={()=>navigation.goBack()}><BackIcon width={24} height={24}/></Pressable><Text style={[styles.title,{color:colors.text}]}>Subscriptions</Text><View style={{width:42}}/></View>
  {loading?<ActivityIndicator color={colors.primary} style={styles.loader}/>:<ScrollView>
   {items.length ? items.map(x=><Pressable key={x.id} onPress={()=>navigation.navigate("SubscriptionDetails",{subscriptionId:x.id})} style={[styles.item,{borderBottomColor:colors.border}]}>
     <View style={[styles.logo,{borderColor:colors.border}]}><ReDomMark width={38} height={38}/></View>
