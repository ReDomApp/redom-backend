...
export function HomeFeedScreenV3() {
  ...
  const refreshFeed = useCallback(async () => { setRefreshing(true); try { const result = await feedService.getHomeFeed(); setPosts(result.posts); setSuggestions(result.suggestedProfiles); setStories(result.friendStories); } finally { setRefreshing(false); } }, []);
  ...
  return <SafeAreaView style={styles.root}>
    ...
    <ScrollView ref={scrollRef} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshFeed}/>} contentContainerStyle={styles.feed}>
      <View style={styles.composer}><Avatar uri={user?.profilePhoto} size={42}/><View style={styles.composerInput}><Text style={styles.composerHint}>What's on your mind{user?.firstName?`, ${user.firstName}`:""}?</Text></View><Pressable><AddMediaIcon width={27} height={27}/></Pressable></View>
