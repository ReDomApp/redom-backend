...
export interface EncryptedMediaEnvelope { version: 1; algorithm: "X25519-AES-256-GCM"; ephemeralPublicKey: string; encryptedMediaKey: string; conversationKeyVersion?: number; }
...
export async function encryptMediaForParticipants(dataUri: string, participantKeys: Array<{ profile_id: string; device_id: string; public_key: string | null }>, context: string) {
  ...
  for (const participant of participantKeys) { if (!participant.public_key || !participant.device_id) throw new Error("Every active ReDom device must have an encryption key before encrypted media can be sent."); const wrapped = await encryptForRecipient(bytesToHex(rawKey), participant.public_key, `${context}|media-key|${participant.device_id}`); envelopes[participant.device_id] = { version: 1, algorithm: wrapped.algorithm, ephemeralPublicKey: wrapped.ephemeralPublicKey, encryptedMediaKey: wrapped.ciphertext, conversationKeyVersion: wrapped.conversationKeyVersion }; }
  return { mime: parsed.mime, ciphertextDataUri: `data:application/octet-stream;base64,${bytesToBase64(combined)}`, envelopes };
}
export async function decryptMediaKey(envelope: EncryptedMediaEnvelope, context: string, deviceId?: string) { const wrapped: ReDomEncryptedEnvelope = { version: 1, algorithm: envelope.algorithm, ephemeralPublicKey: envelope.ephemeralPublicKey, ciphertext: envelope.encryptedMediaKey, conversationKeyVersion: envelope.conversationKeyVersion }; return hexToBytes(await decryptFromSender(wrapped, `${context}|media-key${deviceId ? `|${deviceId}` : ""}`)); }
