...
async function legacyEncryptForRecipient(plaintext: string, recipientPublicKeyHex: string, context: string): Promise<ReDomEncryptedEnvelope> { const ephemeralSecret = randomSecretKey(); const ephemeralPublic = x25519.getPublicKey(ephemeralSecret); const shared = x25519.getSharedSecret(ephemeralSecret, hexToBytes(recipientPublicKeyHex)); const key = await deriveAesKey(shared, context); const combined = encryptAesGcm(new TextEncoder().encode(plaintext), key); return { version: 1, algorithm: CONVERSATION_ALGORITHM, ephemeralPublicKey: bytesToHex(ephemeralPublic), ciphertext: bytesToHex(combined) }; }
...
export async function decryptEnvelopeMap(payload: Record<string, unknown>, context: string): Promise<string | null> { for (const value of Object.values(payload)) { try { if (value && typeof value === "object") return await decryptFromSender(value as ReDomEncryptedEnvelope, context); } catch { /* envelope belongs to another device or key version */ } } return null; }
