...
async function getStoredConversationKey(conversationId: string): Promise<Uint8Array | null> { const stored = await SecureStore.getItemAsync(`${CONVERSATION_KEY_PREFIX}${conversationId}`); return stored ? hexToBytes(stored) : null; }
...
async function establishConversationKey(conversationId: string, participants: CryptoParticipantLike[]): Promise<Uint8Array> {
  const device = await ensureDeviceKey(); let key = await getStoredConversationKey(conversationId); let state: { envelope?: ReDomEncryptedEnvelope; deviceIds?: string[] } | null = null;
  try { state = await api.get<{ success: boolean; envelope?: ReDomEncryptedEnvelope; deviceIds?: string[] }>(`/messages/crypto/conversations/${conversationId}/key?deviceId=${encodeURIComponent(device.deviceId)}`); } catch (error: any) { const status = Number(error?.status ?? error?.response?.status ?? 0); if (status !== 404 && status !== 409) throw error; if (status === 409 && error?.response?.data) state = error.response.data; }
  ...
  if (!key) { key = randomConversationKey(); const envelopes: Record<string, ReDomEncryptedEnvelope> = {}; for (const participant of participants) { if (!participant.public_key) throw new Error("This conversation participant has not registered an encryption device."); envelopes[participant.device_id] = await legacyEncryptForRecipient(bytesToHex(key), participant.public_key, `conversation-key:${conversationId}`); } try { await api.post(`/messages/crypto/conversations/${conversationId}/key/initialize`, { deviceId: device.deviceId, envelopes }); } catch (error: any) { const status = Number(error?.status ?? error?.response?.status ?? 0); if (status !== 409) throw error; } await saveConversationKey(conversationId, key); return key; }
  const covered = new Set(state?.deviceIds ?? []); const missing: Record<string, ReDomEncryptedEnvelope> = {}; for (const participant of participants) { if (!covered.has(participant.device_id)) { if (!participant.public_key) throw new Error("This conversation participant has not registered an encryption device."); missing[participant.device_id] = await legacyEncryptForRecipient(bytesToHex(key), participant.public_key, `conversation-key:${conversationId}`); } } if (Object.keys(missing).length) await api.post(`/messages/crypto/conversations/${conversationId}/key/envelopes`, { deviceId: device.deviceId, envelopes: missing }); return key;
}
...
async function conversationKeyForDecryption(context: string): Promise<Uint8Array> { const conversationId = conversationIdFromContext(context); const local = await getStoredConversationKey(conversationId); if (local) return local; const device = await ensureDeviceKey(); const result = await api.get<{ success: boolean; envelope?: ReDomEncryptedEnvelope }>(`/messages/crypto/conversations/${conversationId}/key?deviceId=${encodeURIComponent(device.deviceId)}`); if (!result.envelope) throw new Error("This device has not received the conversation encryption key."); const key = hexToBytes(await legacyDecryptFromSender(result.envelope, `conversation-key:${conversationId}`)); await saveConversationKey(conversationId, key); return key; }

export async function encryptForRecipient(plaintext: string, _recipientPublicKeyHex: string, context: string): Promise<ReDomEncryptedEnvelope> { const key = await conversationKeyForEncryption(context); return { version: 1, algorithm: CONVERSATION_ALGORITHM, ephemeralPublicKey: ZERO_PUBLIC_KEY, ciphertext: bytesToHex(encryptAesGcm(new TextEncoder().encode(plaintext), key)) }; }
...
export async function decryptEnvelopeMap(payload: Record<string, unknown>, context: string): Promise<string | null> { for (const value of Object.values(payload)) { try { if (value && typeof value === "object") return await decryptFromSender(value as ReDomEncryptedEnvelope, context); } catch { /* another device/key version */ } } return null; }
